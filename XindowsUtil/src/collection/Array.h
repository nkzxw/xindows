...
#ifndef __XINDOWSUTIL_COLLECTION_ARRAY_H__
#define __XINDOWSUTIL_COLLECTION_ARRAY_H__
...
public:
    ...
    inline CImplAry() { memset(this, 0, sizeof(CImplAry)); }
    ~CImplAry();
    inline int  Size() const    { return _c; }
    ...
    inline void* Deref(size_t cb, int i)
    {
        Assert(i >= 0);
    ...
    void LockCheck(BOOL fState) { _fCheckLock = fState; }
#else
    void LockCheck(BOOL) {}
#endif
...
protected:
    ...
    int         FindIndirect(size_t cb, void* pv);
    ...
    UINT        GetStackSize()      { Assert(_fStack); return *(UINT*)((BYTE*)this+sizeof(CImplAry)); }
    void*       GetStackPtr()       { Assert(_fStack); return (void*)((BYTE*)this+sizeof(CImplAry)+sizeof(int)); }
...
//
//-------------------------------------------------------------------------
class XINDOWS_PUBLIC CImplPtrAry : public CImplAry
{
protected:
    DECLARE_MEMALLOC_NEW_DELETE();
...
public:
    ...
    HRESULT     EnumElements(REFIID iid, void** ppv, BOOL fAddRef, BOOL fCopy=TRUE, BOOL fDelete=TRUE);

    HRESULT     EnumVARIANT(VARTYPE vt, IEnumVARIANT** ppenum, BOOL fCopy=TRUE, BOOL fDelete=TRUE);
};
...
//
//----------------------------------------------------------------------------
template<class ELEM> class CDataAry : public CImplAry
{
public:
    DECLARE_MEMALLOC_NEW_DELETE();
    CDataAry() : CImplAry() {}
...
//
//----------------------------------------------------------------------------
template<class ELEM> class CPtrAry : public CImplPtrAry
{
public:
    DECLARE_MEMALLOC_NEW_DELETE();
...
//
//----------------------------------------------------------------------------
template<class ELEM, int N> class CStackDataAry : public CDataAry<ELEM>
{
public:
    DECLARE_MEMALLOC_NEW_DELETE();
...
//
//----------------------------------------------------------------------------
template <class ELEM, int N> class CStackPtrAry : public CPtrAry<ELEM>
{
public:
    DECLARE_MEMALLOC_NEW_DELETE();
...
//
//----------------------------------------------------------------------------
class XINDOWS_PUBLIC CBaseEnum : public IUnknown
{
public:
    DECLARE_MEMALLOC_NEW_DELETE();
    DECLARE_STANDARD_IUNKNOWN(CBaseEnum);
...
//
//----------------------------------------------------------------------------
inline void* CBaseEnum::Deref(int i)
{
    Assert(i >= 0);
...
//
//----------------------------------------------------------------------------
class XINDOWS_PUBLIC CStackCStrAry : public CStackDataAry<CString, 10>
{
public:
    DECLARE_MEMALLOC_NEW_DELETE();
    CStackCStrAry() : CStackDataAry<CString, 10>() {}
    ~CStackCStrAry();
};

#define DECLARE_CStackCStrAry(tAry) \
class tAry : public CStackCStrAry   \
{ public: DECLARE_MEMALLOC_NEW_DELETE(); tAry() : CStackCStrAry() {} };

#endif //__XINDOWSUTIL_COLLECTION_ARRAY_H__