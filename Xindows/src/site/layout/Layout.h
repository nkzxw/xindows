...
#ifndef __XINDOWS_SITE_LAYOUT_LAYOUT_H__
#define __XINDOWS_SITE_LAYOUT_LAYOUT_H__

#include "../display/DispDefs.h"
#include "../display/DispClient.h"
#include "../view/Adorner.h"
#include "../view/View.h"
...
public:
	...
	{
		return  _sp._fHSBAllowed||_sp._fVSBAllowed;
	}
	...
	{
		return (_overflowX>_overflowY ? _overflowX : _overflowY);
	}
...
public:
	...
	DECLARE_STANDARD_IUNKNOWN(CLayout)
	...
	DECLARE_CPtrAry(CRequests, CRequest*)
	...
	void GetClippedClientRect(CRect* prc, CLIENTRECT crt) const
	{
		GetClippedClientRect(prc, COORDSYS_CONTENT, crt);
	}
	void GetClippedClientRect(RECT* prc, CLIENTRECT crt) const
	{
	...
	HRESULT STDMETHODCALLTYPE OnScroll(int iDirection, UINT uCode, long lPosition=0, BOOL fRepeat=FALSE, LONG lScrollTime=0);

	htmlComponent ComponentFromPoint(long x, long y);
	...
	{
		return GetFirstBranch()?GetFirstBranch()->GetCurParentLayout():NULL;
	}
	CLayout* GetUpdatedParentLayout()
	{
		return GetFirstBranch()?GetFirstBranch()->GetUpdatedParentLayout():NULL;
	}
...
//
//-----------------------------------------------------------------------------
inline BOOL CLayout::IsPositionValid()
{
	return _fPositionSet;
}

// Inlines for managing the request queue
inline BOOL CLayout::HasLookasidePtr(int iPtr)
{
	return(_fHasLookasidePtr & (1 << iPtr));
}

inline void* CLayout::GetLookasidePtr(int iPtr)
{
#ifdef _DEBUG
...
inline HRESULT CLayout::SetLookasidePtr(int iPtr, void* pvVal)
{
...
inline void* CLayout::DelLookasidePtr(int iPtr)
{
	if(HasLookasidePtr(iPtr))
	{
...
inline BOOL CLayout::CanHaveBgRecalcInfo()
{
...
inline CBgRecalcInfo* CLayout::BgRecalcInfo()
{
	return HasBgRecalcInfo()?(CBgRecalcInfo*)GetLookasidePtr(LOOKASIDE_BGRECALCINFO):NULL;
}

inline HRESULT CLayout::EnsureBgRecalcInfo()
{
	if(HasBgRecalcInfo())
	{
		return S_OK;
	...
	if(pBgRecalcInfo)
	{
		SetLookasidePtr(LOOKASIDE_BGRECALCINFO, pBgRecalcInfo);
	}

	return pBgRecalcInfo ? S_OK : E_OUTOFMEMORY;
}

inline void CLayout::DeleteBgRecalcInfo()
{
...
inline BOOL CLayout::IsAdorned()
{
	return _fAdorned;
}

inline BOOL CLayout::IsCalcingSize()
{
	return _fCalcingSize;
}

inline void CLayout::SetCalcing(BOOL fState)
{
	_fCalcingSize = fState;
}

inline BOOL CLayout::DoFuzzyHitTest()
{
	return FALSE;
}

#endif //__XINDOWS_SITE_LAYOUT_LAYOUT_H__
