...
#include "../layout/ScrollbarController.h"
...
//
//-----------------------------------------------------------------------------
CRequest::CRequest(REQUESTFLAGS rf, CElement* pElement)
{
    Assert(pElement);
...
//
//-----------------------------------------------------------------------------
DWORD CRequest::AddRef()
{
    Assert(_cRefs > 0);
    return ++_cRefs;
}

DWORD CRequest::Release()
{
    Assert(_cRefs > 0);
    --_cRefs;
    if(!_cRefs)
...
//
//-----------------------------------------------------------------------------
void CRequest::SetFlag(REQUESTFLAGS rf)
{
    _grfFlags |= rf;
}

void CRequest::ClearFlag(REQUESTFLAGS rf)
{
    _grfFlags &= ~rf;
}

BOOL CRequest::IsFlagSet(REQUESTFLAGS rf) const
{
    return _grfFlags & rf;
}
...
//
//-----------------------------------------------------------------------------
BOOL CRequest::QueuedOnLayout(CLayout* pLayout) const
{
    return  (_pLayoutMeasure==pLayout || _pLayoutPosition==pLayout || _pLayoutAdorner==pLayout);
}

void CRequest::DequeueFromLayout(CLayout* pLayout)
{
...
CLayout* CRequest::GetLayout(REQUESTFLAGS rf) const
{
    switch(rf)
    {
    default:
...
void CRequest::SetLayout(REQUESTFLAGS rf, CLayout* pLayout)
{
    switch(rf)
    {
...
//
//-----------------------------------------------------------------------------
CElement* CRequest::GetElement() const
{
    return _pElement;
}

CAdorner* CRequest::GetAdorner() const
{
    return _pAdorner;
}

void CRequest::SetAdorner(CAdorner* pAdorner)
{
    Assert(pAdorner);
...
CPoint& CRequest::GetAuto()
{
    return _ptAuto;
}

void CRequest::SetAuto(const CPoint& ptAuto, BOOL fAutoValid)
{
    if(fAutoValid)
    {
        _ptAuto = ptAuto;
...
CDocument* CLayout::Doc() const
{
    return _fHasMarkupPtr?((CMarkup*)__pvChain)->Doc():(CDocument*)__pvChain;
}
...
//
//-----------------------------------------------------------------------------
void CLayout::Reset(BOOL fForce)
{
    if(fForce)
    {
        RemoveLayoutRequest();
...
//
//-------------------------------------------------------------------------
HRESULT CLayout::QueryInterface(REFIID riid, LPVOID* ppv)
{
    HRESULT hr = S_OK;
...
//
//----------------------------------------------------------------------------
void CLayout::RestrictPointToClientRect(POINT* ppt)
{
    RECT rcClient;
...
{
    Assert(prc);
    Assert(crt!=CLIENTRECT_USERECT || cs==COORDSYS_CONTENT);
    ...
    else
    {
        ...
        if(crt != CLIENTRECT_BACKGROUND)
        {
            ...
            {
                prc->right -= _afxGlobalData._sizeScrollbar.cx;
            }
        }

        if(dni.IsHScrollbarForced())
        {
            prc->bottom -= _afxGlobalData._sizeScrollbar.cy;
        }
...
//
//-------------------------------------------------------------------------
void CLayout::GetClippedClientRect(
    CRect*				prc,
    COORDINATE_SYSTEM	cs,
...
//
//--------------------------------------------------------------------------
DWORD CLayout::CalcSize(CCalcInfo* pci, SIZE* psize, SIZE* psizeDefault)
{
    Assert(pci);
    Assert(psize);
    Assert(ElementOwner());
    ...
    if(pci->_smMode != SIZEMODE_SET)
    {
        if(_fSizeThis)
        {
            ...
            if(!psizeDefault)
            {
                psizeDefault = &_afxGlobalData._Zero.size;
            }
			...
			_fContentsAffectSize = ((uvx.IsNullOrEnum() || uvy.IsNullOrEnum())
    ...
    case SIZEMODE_FULLSIZE:
        ...
        *psize = size;
		...
		grfReturn |= LAYOUT_THIS |
			(size.cx!=sizeOriginal.cx?LAYOUT_HRESIZE:0) |
			(size.cy!=sizeOriginal.cy?LAYOUT_VRESIZE:0);
    ...
    case SIZEMODE_MMWIDTH:
        ...
        psize->cx = psize->cy =
            (pTreeNode->GetCascadedwidth().GetUnitType()==CUnitValue::UNIT_PERCENT
            ? 0 : size.cx);
        break;

    case SIZEMODE_PAGE:
        // Use the object's size if it fits, otherwise use zero
		psize->cx = (size.cx<psize->cx ? size.cx : 0);
		psize->cy = (size.cy<psize->cy ? size.cy : 0);
		break;
	...
	Trace((_T("CalcSize  : Layout(0x%x, %S) size(%d,%d) mode(%S)\n"),
		this,
...
//
//-----------------------------------------------------------------------------
void CLayout::GetRect(CRect* prc, COORDINATE_SYSTEM cs) const
{
    Assert(prc);
...
//
//-----------------------------------------------------------------------------
void CLayout::GetClippedRect(CRect* prc, COORDINATE_SYSTEM cs) const
{
    Assert(prc);
...
//
//-----------------------------------------------------------------------------
void CLayout::GetSize(CSize* psize) const
{
    Assert(psize);
...
//
//-----------------------------------------------------------------------------
void CLayout::GetContentSize(CSize* psize, BOOL fActualSize) const
{
    CDispNode* pDispNode = GetElementDispNode();
...
//
//-----------------------------------------------------------------------------
void CLayout::GetPosition(CPoint* ppt, COORDINATE_SYSTEM cs) const
{
    Assert(ppt);
...
//
//-----------------------------------------------------------------------------
void CLayout::SetPosition(const CPoint& pt, BOOL fNotifyAuto)
{
    CPoint ptOriginal;
...
//
//---------------------------------------------------------------------------
void CLayout::HandleTranslatedRange(const CSize& size)
{
    Assert(_fContainsRelative || ElementOwner()->IsZParent());

    if((_fContainsRelative || GetFirstBranch()->GetFancyFormat()->_fAutoPositioned))
    {
...
//
//-----------------------------------------------------------------------------
CRequest* CLayout::QueueRequest(CRequest::REQUESTFLAGS rf, CElement* pElement)
{
    Assert(pElement);
	...
	Assert(!TestLock(CElement::ELEMENTLOCK_PROCESSPOSITION) ||
        (rf!=CRequest::RF_MEASURE && rf!=CRequest::RF_POSITION));
    ...
    if(!pRequests)
    {
        ...
        if(!pRequests || !SUCCEEDED(AddRequestQueue(pRequests)))
        {
            delete pRequests;
            goto Error;
        }
    ...
    if(!pRequest)
    {
        ...
        if(!pRequest || !SUCCEEDED(pElement->SetRequestPtr(pRequest)))
        {
            goto Error;
        }
    }
    ...
    if(fQueueRequest && !SUCCEEDED(pRequests->Append(pRequest)))
	{
		goto Error;
	}
	...
	PostLayoutRequest(rf==CRequest::RF_MEASURE
		? LAYOUT_MEASURE : rf==CRequest::RF_POSITION
		? LAYOUT_POSITION : LAYOUT_ADORNERS);
...
//
//-----------------------------------------------------------------------------
BOOL CLayout::ProcessRequest(CCalcInfo* pci, CRequest* pRequest)
{
    Assert(pci);
    ...
    {
        if(pRequest->IsFlagSet(CRequest::RF_MEASURE) && pRequest->GetLayout(CRequest::RF_MEASURE)==this)
        {
            if(pci->_grfLayout & LAYOUT_MEASURE)
            {
				...
				HandleElementMeasureRequest(pci, 
					pRequest->GetElement(), ElementOwner()->IsEditable(TRUE));
        ...
        if(pRequest->IsFlagSet(CRequest::RF_POSITION) && pRequest->GetLayout(CRequest::RF_POSITION)==this)
        {
...
void CLayout::ProcessRequest(CElement* pElement)
{
    if(pElement && !pElement->IsPositionStatic()) 
    {
        ...
        if(pRequest)
        {
            ...
            if(!pRequest->IsFlagSet(CRequest::RF_ADDADORNER))
            {
                ...
                if(pRequests && pRequests->DeleteByValue(pRequest))
                {
                    pRequest->DequeueFromLayout(this);
                    pRequest->Release();
                }

                if(pRequests && !pRequests->Size())
                {
                    DeleteRequestQueue();
                    delete pRequests;
                }
...
//
//-----------------------------------------------------------------------------
void CLayout::ProcessRequests(CCalcInfo* pci, const CSize& size)
{
    Assert(pci);
...
//
//----------------------------------------------------------------------------
HTC CLayout::HitTestPoint(const CPoint& pt, CTreeNode** ppNodeElement, DWORD grfFlags)
{
...
//
//----------------------------------------------------------------------------
BOOL CLayout::HitTestRect(RECT* prc)
{
    // BUGBUG: Should this routine be used any longer? (brendand)
    return FALSE;
}

HRESULT CLayout::GetChildElementTopLeft(POINT& pt, CElement* pChild)
{
...
void CLayout::GetMarginInfo(CParentInfo* ppri,
                       ...
                       LONG* plRightMargin, LONG* plBottomMargin)
{
    ...
    if(!ElementOwner()->IsBlockElement() || !ElementOwner()->IsInlinedElement())
    {
...
//
//-------------------------------------------------------------------------
void CLayout::SetIsAdorned(BOOL fAdorned)
{
    _fAdorned = fAdorned;
}
...
//
//-------------------------------------------------------------------------
HRESULT CLayout::PreDrag(DWORD dwKeyState, IDataObject** ppDO, IDropSource** ppDS)
{
    RRETURN(E_FAIL);
}
...
//
//-------------------------------------------------------------------------
HRESULT CLayout::PostDrag(HRESULT hrDrop, DWORD dwEffect)
{
    RRETURN(E_FAIL);
}

static HRESULT CreateDataObject(CDocument* pDoc, IUniformResourceLocator* pUrlToDrag, IDataObject** ppDataObj)
{
    HRESULT hr;
    IDataObject* pLinkDataObj = NULL;
    CGenDataObject* pDataObj;

    hr = pUrlToDrag->QueryInterface(IID_IDataObject, (void**)&pLinkDataObj);
    if(hr)
	{
		goto Cleanup;
	}
...
//
//-------------------------------------------------------------------------
HRESULT CLayout::DoDrag(DWORD dwKeyState,
                ...
                BOOL fCreateDataObjOnly/*=FALSE*/)
{
    ...
    CDocument*      pDoc        = Doc();
    /*CParentUndoUnit* pPUU = NULL; wlw note*/
    CDragStartInfo* pDragStartInfo = pDoc->_pDragStartInfo;
    CLayout*		pTopLayout;

    if(!fCreateDataObjOnly)
	{
		/*pPUU = pDoc->OpenParentUnit(pDoc, IDS_UNDODRAGDROP); wlw note*/
	}
    ...
    if(fCreateDataObjOnly || !pDragStartInfo || !pDragStartInfo->_pDataObj)
    {
        ...
        if(pURLToDrag)
        {
            ...
            if(!hr)
			{
				hr = CDummyDropSource::Create(dwKeyState, pDoc, &pDropSource);
			}
        }
        else
		{
			hr = PreDrag(dwKeyState, &pDataObj, &pDropSource);
		}
        if(hr)
        {
            if(S_FALSE == hr)
    ...
    if(ElementOwner()->IsEditable(TRUE) && pDoc->_pElemCurrent && pDoc->_pElemCurrent->GetHwnd())
    {
    ...
    hr = DoDragDrop(pDataObj, pDropSource, dwEffectAllowed, &dwEffect);
    ...
    if(DRAGDROP_S_DROP==hr && DROPEFFECT_NONE!=dwEffect && !(dwEffect&dwEffectAllowed))
    {
    ...
    if(!pURLToDrag && ElementOwner()->IsInMarkup()
        && (!pDragStartInfo||pDragStartInfo->_pElementDrag->GetFirstBranch()))
    {
        hr = PostDrag(hr, dwEffect);
        if(hr)
		{
			goto Cleanup;
		}
...
Cleanup:
    ...
    if(!fCreateDataObjOnly)
    {
        /*pDoc->CloseParentUnit(pPUU, S_OK); wlw note*/
...
//
//-------------------------------------------------------------------------
HRESULT CLayout::DropHelper(POINTL ptlScreen, DWORD dwAllowed, DWORD* pdwEffect, LPTSTR lptszFileType)
{
    HRESULT hr = S_OK;
    ...
    if(Doc()->_fRightBtnDrag)
    {
        ...
        *pdwEffect = DROPEFFECT_NONE;
        ...
        if(!Doc()->_fSlowClick)
        {
            hr = Doc()->ShowDragContextMenu(ptlScreen, dwAllowed, &iSelection, lptszFileType);
            if(S_OK == hr)
            {
        ...
        DWORD* pdwEffect)
{
    ...
    hr = InitDragInfo(pDataObj, ptlScreen);
    if(hr)
	{
		goto Cleanup;
	}

    hr = DragOver(grfKeyState, ptlScreen, pdwEffect);
...
//
//----------------------------------------------------------------------------
HRESULT CLayout::ParseDragData(IDataObject* pDataObj)
{
...
//
//----------------------------------------------------------------------------
HRESULT CLayout::DragOver(DWORD grfKeyState, POINTL ptlScreen, LPDWORD pdwEffect)
{
    HRESULT hr = S_OK;
    ...
    else if(pDoc->_fFromCtrlPalette)
    {
        *pdwEffect &= DROPEFFECT_COPY; // Drag from Control Palette
		...
		pDoc->_fOKLink && (*pdwEffect&DROPEFFECT_LINK))
	{
        *pdwEffect = DROPEFFECT_LINK; // Control-Shift equals create link
    }
    else if(grfKeyState==MK_CONTROL && (*pdwEffect&DROPEFFECT_COPY))
    {
    ...
    else if((pDoc->_fOKLink) && (*pdwEffect&DROPEFFECT_LINK))
    {
    ...
    {
        hr = UpdateDragFeedback(ptlScreen);
        if(pDoc->_fSlowClick)
        {
            *pdwEffect = DROPEFFECT_NONE ;
        }
...
//
//----------------------------------------------------------------------------
BOOL CLayout::OnSetCursor(CMessage* pMessage)
{
...
//
//----------------------------------------------------------------------------
void CLayout::DrawZeroBorder(CFormDrawInfo* pDI)
{
...
void DrawTextSelectionForRect(HDC hdc, CRect* prc, CRect* prcClip, BOOL fSwapColor)
{
    ...
    crOldBk = SetBkColor(hdc,
		GetSysColor(fSwapColor?COLOR_HIGHLIGHTTEXT:COLOR_HIGHLIGHT));
    ...
    PatBlt(hdc, prc->left, prc->top, prc->right-prc->left, prc->bottom-prc->top, DST_PAT_OR);            
    ...
    PatBlt(hdc, prc->left, prc->top, prc->right-prc->left, prc->bottom-prc->top, DST_PAT_AND);   
...
//
//----------------------------------------------------------------------------
void CLayout::DrawTextSelectionForSite(CFormDrawInfo* pDI)
{
    if(_fTextSelected)
    {
        CRect rcContent;
...
//
//----------------------------------------------------------------------------
void CLayout::Draw(CFormDrawInfo* pDI, CDispNode*)
{
    return;
}

void CLayout::DrawBackground(CFormDrawInfo* pDI, BACKGROUNDINFO* pbginfo, RECT* prcDraw)
{
    CRect		rcBackground;
    SIZE		sizeImg;
    CDocument*  pDoc = Doc();
	...
	if(crBack!=CLR_INVALID
		&& (!pImgCtx || !(ulState&IMGTRANS_OPAQUE) || !EqualRect(&rcBackground, &pbginfo->rcImg)))
	{
...
HRESULT CLayout::GetElementsInZOrder(
...
HRESULT CLayout::GetDC(LPRECT prc, DWORD dwFlags, HDC* phDC)
{
    CDocument* pDoc = Doc();
	...
	dwFlags |= ((pDoc->_bufferDepth&OFFSCR_BPP)<<16) | (pDoc->_cSurface?OFFSCR_SURFACE:0)
...
HRESULT CLayout::ReleaseDC(HDC hdc)
{
...
//
//------------------------------------------------------------------------
void CLayout::Invalidate(const RECT& rc, COORDINATE_SYSTEM cs)
{
    if(GetFirstBranch() && _pDispNode)
    {
        _pDispNode->Invalidate((const CRect&)rc, cs);
    }
}

void CLayout::Invalidate(LPCRECT prc, int cRects, LPCRECT prcClip)
{
    if(GetFirstBranch() && _pDispNode)
    {
        if(!prc)
...
void CLayout::Invalidate(HRGN hrgn)
{
    if(GetFirstBranch() && _pDispNode)
    {
...
//
//-------------------------------------------------------------------------
BOOL CLayout::GetBackgroundInfo(
    CFormDrawInfo*		pDI,
    ...
    BOOL				fRightToLeft)
{
    ...
    pbginfo->fFixed				= (pbginfo->pImgCtx && pFF->_fBgFixed);

    pbginfo->crBack  = cv.IsDefined() ? cv.GetColorRef() : CLR_INVALID;
    pbginfo->crTrans = CLR_INVALID;

    if(fAll && pbginfo->pImgCtx)
    {
...
//
//-------------------------------------------------------------------------
void CLayout::AttachScrollbarController(CDispNode* pDispNode, CMessage* pMessage)
{
    CScrollbarController::StartScrollbarController(
        this,
        DYNCAST(CDispScroller, pDispNode),
        Doc(),
        _afxGlobalData._sizeScrollbar.cx,
        pMessage);
}

void CLayout::DetachScrollbarController(CDispNode* pDispNode)
{
    CScrollbarController* pSBC = TLS(pSBC);

    if(pSBC && pSBC->GetLayout()==this)
    {
...
//
//-------------------------------------------------------------------------
HRESULT CLayout::ScrollElementIntoView(
									   CElement*	pElement,
									   SCROLLPIN	spVert,
									   SCROLLPIN	spHorz,
									   BOOL			fScrollBits)
    ...
    if(IsFlowLayout())
    {
        ...
        if(pElement != ElementOwner())
        {
            ...
            else
            {
                ...
                Assert(pElement->GetUpdatedLayout() && " your are about to crash");
...
//
//----------------------------------------------------------------------------
HRESULT CLayout::ScrollRangeIntoView(
									 long		cpMin,
									 long		cpMost,
									 SCROLLPIN	spVert,
									 SCROLLPIN	spHorz,
...
//
//----------------------------------------------------------------------------
BOOL CLayout::ScrollRectIntoView(
								 const CRect&	rc,
								 SCROLLPIN		spVert,
								 SCROLLPIN		spHorz,
								 BOOL			fScrollBits)
...
STDMETHODIMP CLayout::HandleMessage(CMessage* pMessage)
{
    HRESULT		hr			= S_FALSE;
    CDispNode*	pDispNode	= GetElementDispNode();
...
//        hr = ElementOwner()->OnInitMenuPopup(
...
//#endif // NO_MENU
    ...
    case WM_KEYDOWN:
        // BUGBUG (MohanB) how to get the child here?
        hr = HandleKeyDown(pMessage, ElementOwner());
        break;
    ...
    case WM_CHAR:
        ...
        {
            hr = pDoc->ActivateDefaultButton(pMessage);
            break;
        }

        if(fInBrowse && pMessage->wParam==VK_SPACE && fIsScroller)
        {
            OnScroll(
                1,
                pMessage->dwKeyState&MK_SHIFT ? SB_PAGEUP : SB_PAGEDOWN,
                0,
                FALSE,
...
//
//--------------------------------------------------------------------------
void CLayout::PrepareMessage(CMessage* pMessage, CDispNode* pDispNode)
{
...
void CLayout::AdjustSizeForBorder(SIZE* pSize, CDocInfo* pdci, BOOL fInflate)
{
    CBorderInfo bInfo;

	if(ElementOwner()->GetBorderInfo(pdci, &bInfo, FALSE))
	{
		int iXWidths = pdci->DocPixelsFromWindowX(
			bInfo.aiWidths[BORDER_RIGHT]+bInfo.aiWidths[BORDER_LEFT], FALSE);
		int iYWidths = pdci->DocPixelsFromWindowY(
			bInfo.aiWidths[BORDER_TOP]+bInfo.aiWidths[BORDER_BOTTOM], FALSE);
...
//
//----------------------------------------------------------------------------
HRESULT CLayout::Move(RECT* prcpixels, DWORD dwFlags)
{
    HRESULT		hr = S_OK;
    RECT		rcWindow;
    ...
    if(pParentLayout)
    {
        if(pParentLayout->GetElementDispNode() && pParentLayout->GetElementDispNode()->IsScroller())
        {
    ...
    if(!(dwFlags & SITEMOVE_NORESIZE))
    {
        ...
        // Set Attributes
		hr = ElementOwner()->SetDim(STDPROPID_XOBJ_HEIGHT,
		...
		hr = ElementOwner()->SetDim(STDPROPID_XOBJ_WIDTH,
    ...
    {
        hr = ElementOwner()->SetDim(STDPROPID_XOBJ_TOP,
                            (float)rcWindow.top,
                            CUnitValue::UNIT_PIXELS,
                            1,
                            NULL,
        ...
        hr = ElementOwner()->SetDim(STDPROPID_XOBJ_LEFT,
                            (float)rcWindow.left,
                            CUnitValue::UNIT_PIXELS,
                            1,
                            NULL,
...
//
//----------------------------------------------------------------------------
void CLayout::SetSurfaceFlags(BOOL fSurface, BOOL f3DSurface, BOOL fDontFilter /* = FALSE */)
{
...
//
//--------------------------------------------------------------------------
void CLayout::SetSiteTextSelection(BOOL fSelected, BOOL fSwap)
{
...
//
//----------------------------------------------------------------------------
HRESULT CLayout::HandleKeyDown(CMessage* pMessage, CElement* pElemChild)
{
    ...
    if(hr==S_FALSE && !fAlt && !fCtrl)
    {
        ...
        case VK_ESCAPE:
            if(fRunMode)
            {
                hr = Doc()->ActivateCancelButton(pMessage);
            }
            break;
        }
    }
...
//
//----------------------------------------------------------------------------
void CLayout::GetOwner(CDispNode* pDispNode, void** ppv)
{
    Assert(pDispNode);
...
//
//----------------------------------------------------------------------------
void CLayout::DrawClientBackground(
    const RECT*		prcBounds,
    const RECT*		prcRedraw,
    CDispSurface*	pDispSurface,
    CDispNode*		pDispNode,
    ...
    DWORD			dwFlags)
{
    ...
    if(bi.crBack!=CLR_INVALID || bi.pImgCtx)
	{
...
//
//----------------------------------------------------------------------------
void CLayout::DrawClientScrollbar(
    int				iDirection,
    const RECT*		prcBounds,
    const RECT*		prcRedraw,
    long			contentSize,
    ...
    DWORD			dwFlags)
{
    ...
    CScrollbar::Draw(
        ...
        containerSize,
        scrollAmount,
        (pSBC->GetLayout()==this ? pSBC->GetPartPressed() : CScrollbar::SBP_NONE),
        hdc,
        params,
        pDI,
        dwFlags);
...
//
//----------------------------------------------------------------------------
void CLayout::DrawClientScrollbarFiller(
    const RECT*		prcBounds,
    const RECT*		prcRedraw,
    CDispSurface*	pDispSurface,
    CDispNode*		pDispNode,
...
//
//----------------------------------------------------------------------------
BOOL CLayout::HitTestScrollbarFiller(
    const POINT*	pptHit,
    CDispNode*		pDispNode,
    void*			pClientData)
{
...
//
//----------------------------------------------------------------------------
BOOL CLayout::ProcessDisplayTreeTraversal(void* pClientData)
{
    return TRUE;
}
...
//
//----------------------------------------------------------------------------
LONG CLayout::GetZOrderForChild(void* cookie)
{
    return 0;
}
...
//
//----------------------------------------------------------------------------
LONG CLayout::CompareZOrder(CDispNode* pDispNode1, CDispNode* pDispNode2)
{
    Assert(pDispNode1);
    Assert(pDispNode2);
    ...
    return (pElement1!=pElement2 ? pElement1->CompareZOrder(pElement2) : -1);
}
...
//
//----------------------------------------------------------------------------
void CLayout::NotifyScrollEvent(RECT* prcScroll, SIZE* psizeScrollDelta)
{
...
//
//-----------------------------------------------------------------------------
DWORD CLayout::GetClientLayersInfo(CDispNode* pDispNodeFor)
{
...
//
//-----------------------------------------------------------------------------
CDispNode* CLayout::GetElementDispNode(CElement* pElement) const
{
    Assert(!pElement || pElement==ElementOwner());
...
//
//-----------------------------------------------------------------------------
void CLayout::SetElementDispNode(CElement* pElement, CDispNode* pDispNode)
{
    Assert(!pElement || pElement==ElementOwner());
...
//
//-----------------------------------------------------------------------------
CDispItemPlus* CLayout::GetFirstContentDispNode(CDispNode* pDispNode) const
{
    if(!pDispNode)
	{
		pDispNode = GetElementDispNode();
    ...
    pDispNode = (pDispNode&&pDispNode->IsContainer()
        ? DYNCAST(CDispContainer, pDispNode)->GetFirstChildNodeInLayer(DISPNODELAYER_FLOW)
        : NULL);

    while(pDispNode && pDispNode->IsOwned())
    {
		pDispNode = pDispNode->GetNextSiblingNode(TRUE);
	}
	Assert(!pDispNode ||
        (pDispNode->GetNodeType()==DISPNODETYPE_ITEMPLUS && !pDispNode->IsOwned()));

    return (pDispNode ? DYNCAST(CDispItemPlus, pDispNode) : NULL);
}
...
//
//-----------------------------------------------------------------------------
void CLayout::GetDispNodeInfo(CDispNodeInfo* pdni, CDocInfo* pdci, BOOL fBorders) const
{
    CElement*           pElement    = ElementOwner();
    ...
    if(TestLayoutDescFlag(LAYOUTDESC_TABLECELL))
    {
		...
		pdni->_fHasInset = (fVAlign!=htmlCellVAlignNotSet && fVAlign!=htmlCellVAlignTop);
    }
    else
    {
	...
	pdni->_fHasBackground = (bi.crBack!=CLR_INVALID || bi.pImgCtx) ||
...
//
//-----------------------------------------------------------------------------
void CLayout::GetDispNodeScrollbarProperties(CDispNodeInfo* pdni) const
{
...
//
//-----------------------------------------------------------------------------
HRESULT CLayout::EnsureDispNode(CDocInfo* pdci, BOOL fForce)
{
    ...
    if(pDispNodeElement->IsContainer() && !dni.IsTag(ETAG_TR))
    {
            ...
            || dni.IsRTL()!=pDispNodeContent->IsRightToLeft())
        {
            ...
            Assert((CPoint&)pDispNodeContent->GetPosition() == (CPoint&)_afxGlobalData._Zero.pt);
            ...
            else
            {
                ...
                if(fDirectionChanged)
                {
                    ...
                    {
                        pDispNodeContent->SetPositionTopRight(_afxGlobalData._Zero.pt);
                    }
                    else
                    {
                        pDispNodeContent->SetPosition(_afxGlobalData._Zero.pt);
                    }
                }
            }
...
//
//-----------------------------------------------------------------------------
void CLayout::EnsureDispNodeLayer(DISPNODELAYER layer, CDispNode* pDispNode)
{
    if(!pDispNode)
	{
		pDispNode = GetElementDispNode();
    ...
    if(pDispNode && pDispNode->GetLayerType()!=layer)
    {
...
//
//-----------------------------------------------------------------------------
void CLayout::EnsureDispNodeBackground(const CDispNodeInfo& dni, CDispNode* pDispNode)
{
    if(!pDispNode)
    {
        pDispNode = GetElementDispNode();
    ...
    if(pDispNode)
    {
		...
		BOOL fPaintBackground = dni.HasBackground()
			&& (Doc()->PaintBackground() || Tag()==ETAG_TEXTAREA);
...
//
//-----------------------------------------------------------------------------
CDispNode* EnsureContentNode(CDispNode* pDispNode)
{
...
//
//-----------------------------------------------------------------------------
CDispContainer* CLayout::EnsureDispNodeIsContainer(CElement* pElement)
{
    Assert(!pElement || pElement->GetUpdatedNearestLayout()==this);
    Assert(!pElement || pElement==ElementOwner() || !pElement->NeedsLayout());
    ...
    Assert((pElement && pElement!=ElementOwner()) || !GetFirstContentDispNode());
    ...
    {
        Assert(!pElement->NeedsLayout() && pElement->IsRelative());
...
//
//-----------------------------------------------------------------------------
void CLayout::EnsureDispNodeScrollbars(
    CDocInfo*					pdci,
    ...
    CDispNode*					pDispNode)
{
    ...
    if(pDispNode && pDispNode->IsScroller())
    {
        ...
        long cySB = pdci->DocPixelsFromWindowY(sp._fHSBAllowed ? _afxGlobalData._sizeScrollbar.cy : 0);
        long cxSB = pdci->DocPixelsFromWindowX(sp._fVSBAllowed ? _afxGlobalData._sizeScrollbar.cx : 0);
...
//
//-----------------------------------------------------------------------------
void CLayout::EnsureDispNodeVisibility(CElement* pElement, CDispNode* pDispNode)
{
    if(!pElement)
	{
		pElement = ElementOwner();
...
void CLayout::EnsureDispNodeVisibility(
    ...
    CDispNode*		pDispNode)
{
    ...
    if(pDispNode)
    {
		...
		if(visibilityMode==VISIBILITYMODE_INVISIBLE && pElementParent->IsEditable())
		{
...
//
//-----------------------------------------------------------------------------
void CLayout::EnsureContentVisibility(CDispNode* pDispNode, BOOL fVisible)
{
    ...
    BOOL		fRestrictToLayer)
{
    ...
    if(!pDispNodeEnd)
    {
		for(pDispNode=pDispNodeStart;
			pDispNode; pDispNode=pDispNode->GetNextSiblingNode(fRestrictToLayer))
		{
    ...
    } while(pDispNodeEnd && pDispNode!=pDispNodeStart);
...
//
//-----------------------------------------------------------------------------
void CLayout::SetPositionAware(BOOL fPositionAware, CDispNode* pDispNode)
{
    if(!pDispNode)
	{
		pDispNode = GetElementDispNode();
...
//
//-----------------------------------------------------------------------------
void CLayout::SetInsertionAware(BOOL fInsertionAware, CDispNode* pDispNode)
{
    if(!pDispNode)
	{
		pDispNode = GetElementDispNode();
    ...
    BOOL		fInvalidateAll)
{
    ...
    if(!fInvalidateAll && pDispNodeElement->HasBackground())
    {
    ...
    // Fire related events
	if((CSize&)size!=sizeOriginal && !IsDisplayNone()
		&& pDoc->_fFiredOnLoad)
    {
...
//
//-----------------------------------------------------------------------------
void CLayout::SizeDispNodeInsets(
    ...
    CDispNode*			pDispNode)
{
    ...
    if(pDispNode && pDispNode->HasInset())
    {
        ...
        case styleVerticalAlignTop:
            sizeInset = _afxGlobalData._Zero.size;
            break;
...
//
//-----------------------------------------------------------------------------
void CLayout::SizeDispNodeUserClip(
    CDocInfo*      pdci,
    const CSize&   size,
    CDispNode*     pDispNode)
{
...
//
//-----------------------------------------------------------------------------
void CLayout::SizeContentDispNode(const SIZE& size, BOOL fInvalidateAll)
{
    ...
    if(pDispContent)
    {
        ...
        if(pView->IsFlagSet(CView::VF_ZEROBORDER)/* && ElementOwner()->IsEditable()*/)
        {
...
//
//-----------------------------------------------------------------------------
void CLayout::TranslateDispNodes(
    const SIZE&	size,
    CDispNode*	pDispNodeStart,
    CDispNode*	pDispNodeEnd,
    BOOL		fRestrictToLayer,
    BOOL		fExtractHidden)
{
    ...
    if(!pDispNode->IsContainer() || !DYNCAST(CDispContainer, pDispNode)->CountChildren())
	{
		goto Cleanup;
	}

    if(!size.cx && !size.cy && !fExtractHidden)
	{
		goto Cleanup;
	}
...
//
//----------------------------------------------------------------------------
HRESULT CLayout::HandleScrollbarMessage(CMessage* pMessage, CElement* pElement)
{
	CDispNode*	pDispNode	= GetElementDispNode();
	CDocument*  pDoc		= Doc();
	HRESULT		hr			= S_FALSE;

	if(!pDispNode || !ElementOwner()->IsEnabled())
	{
		return hr;
	}
    ...
    case SB_PAGEDOWN:
        {
            ...
            else
            {
                ...
                size = _afxGlobalData._Zero.size;
                size[iDirection] = (uCode==SB_PAGEUP?-1:1) *
                    max(1L, rc.Size(iDirection)-(((CSize &)g_sizeSystemChar)[iDirection]*2L));
...
//
//-----------------------------------------------------------------------------
BOOL CLayout::ScrollBy(const CSize& sizeDelta, LONG lScrollTime)
{
    CDispNode* pDispNode = GetElementDispNode();
    BOOL fRet = FALSE;

    if(pDispNode && pDispNode->IsScroller() && sizeDelta!=_afxGlobalData._Zero.size)
    {
        CSize sizeOffset;
...
BOOL CLayout::ScrollByPercent(const CSize& sizePercent, LONG lScrollTime)
{
    CDispNode*	pDispNode	= GetElementDispNode();
    BOOL		fRet		= FALSE;

	if(pDispNode && pDispNode->IsScroller() && sizePercent!=_afxGlobalData._Zero.size)
	{
		CRect rc;
		CSize sizeOffset;
		CSize sizeDelta;
...
BOOL CLayout::ScrollTo(const CSize& sizeOffset, LONG lScrollTime)
{
    CDispNode*	pDispNode	= GetElementDispNode();
    BOOL		fRet		= FALSE;
...
void CLayout::ScrollToX(long x, LONG lScrollTime)
{
    CDispNode* pDispNode = GetElementDispNode();
...
void CLayout::ScrollToY(long y, LONG lScrollTime)
{
    CDispNode* pDispNode = GetElementDispNode();
...
//
//-----------------------------------------------------------------------------
void CLayout::DoLayout(DWORD grfLayout)
{
...
//
//-----------------------------------------------------------------------------
void CLayout::Notify(CNotification* pnf)
{
    ...
    BOOL		fAutoValid)
{
        ...
        || pElementZParent->GetFirstCp()<pElementLParent->GetFirstCp())
    {
        ...
        if(pElementZParent != pElementLParent)
        {
            ...
            while(pDispNodeParent && pDispNodeParent!=*ppDNZParent)
            {
    ...
    {
        CPoint pt = _afxGlobalData._Zero.pt;
...
//
//-----------------------------------------------------------------------------
void CLayout::HandleVisibleChange(BOOL fVisibility)
{
	...
	if(hwnd && Doc()->_pInPlace)
	{
		...
		CRect rc;
		UINT uFlags = (!fDisplayNone && !fHidden && (!pDispNode||pDispNode->IsInView())
    ...
    if(!fVisibility)
    {
        ...
        if(!fDisplayNone && IsDirty() && !_fSizeThis)
        {
...
//
//------------------------------------------------------------------------------------
void CLayout::HandleZeroGrayChange(CNotification* pnf)
{
    BACKGROUNDINFO bi;
    ...
    else
    {
        ...
        if(_pDispNode)
        {
            ...
            BOOL fHasBack = (bi.crBack!=CLR_INVALID || bi.pImgCtx);
            if(!fHasBack)
			{
...
//
//-----------------------------------------------------------------------------
void CLayout::HandleElementMeasureRequest(CCalcInfo* pci, CElement* pElement, BOOL fEditable)
{
    Assert(pci);
    Assert(pElement);
    ...
    if(pLayout && pTreeNode->GetCascadedposition()==stylePositionabsolute)
    {
        Assert(pElement->GetUpdatedParentLayout() == this);

        if(!pTreeNode->IsDisplayNone() || fEditable)
        {
        ...
        else
        {
            ...
            if(pLayout && pLayout->GetElementDispNode())
            {
...
void CLayout::CalcAbsolutePosChild(CCalcInfo* pci, CLayout* pChildLayout)
{
...
//
//-----------------------------------------------------------------------------
BOOL CLayout::HandlePositionNotification(CNotification* pnf)
{
...
void CLayout::HandlePositionRequest(
    CElement*		pElement,
    const CPoint&	ptAuto,
    BOOL			fAutoValid)
{
    ...
    // relativeness).
    if(!IsDisplayNone() && !pCF->IsDisplayNone()
        && (fAbsolute||pCF->_fRelative||fRelative))
    {
        ...
        if(pDNZParent)
        {
            ...
            if(pDNElement)
            {
                CDocument*  pDoc          = pElementOwner->Doc();
                ...
                BOOL		fRTLParent;
                CPoint		pt(_afxGlobalData._Zero.pt);
                CRect		rc(_afxGlobalData._Zero.rc);
                CSize size;
                ...
                if(!fAbsolute || fLeftAuto || fTopAuto)
                {
                    pt = ptAuto;
                ...
                if(pFF->IsPositioned())
                {
					...
					pDNElement->SetLayerType(pFF->_lZIndex>=0
						?DISPNODELAYER_POSITIVEZ:DISPNODELAYER_NEGATIVEZ);
...
//
//-----------------------------------------------------------------------------
BOOL CLayout::HandleAddAdornerNotification(CNotification* pnf)
{
    CElement*	pElement	= ElementOwner();
    CTreeNode*	pTreeNode	= pElement->GetFirstBranch();
    BOOL		fHandle		= FALSE;
    ...
    else
    {
		...
		BOOL fZParent = fAtTop || pTreeNode->IsZParent()
		...
		if(fAtTop || fZParent)
		{
            ...
            Assert(adl!=ADL_TOPOFFLOW || pAdorner->GetElement()->IsPositionStatic());

			fHandle = (adl==ADL_ALWAYSONTOP&&fAtTop) || (adl==ADL_ONELEMENT&&fZParent)
				|| adl==ADL_TOPOFFLOW;
		}
    }
...
void CLayout::HandleAddAdornerRequest(CAdorner* pAdorner)
{
    Assert(pAdorner);
...
HRESULT CLayout::OnPropertyChange(DISPID dispid, DWORD dwFlags)
{
    HRESULT hr = S_OK;;
...
CMarkup* CLayout::GetContentMarkup() const
{
...
CElement* CLayout::ElementContent() const
{
    if(ElementOwner()->HasSlaveMarkupPtr())
    {
...
//
//----------------------------------------------------------------------------
long CLayout::GetContentTextLength()
{
    if(ElementOwner()->HasSlaveMarkupPtr())
    {
...
long CLayout::GetContentFirstCp()
{
    if(ElementOwner()->HasSlaveMarkupPtr())
    {
...
void CLayout::GetContentTreeExtent(CTreePos** pptpStart, CTreePos** pptpEnd)
{
    if(ElementOwner()->HasSlaveMarkupPtr())
    {
...
//
//-----------------------------------------------------------------------------
void CLayout::RegionFromElement(CElement* pElement,
								...
								DWORD  dwFlags)
{
    ...
    if(_pElementOwner == pElement)
    {
        ...
        else
        {
            ...
            GetRect(prcBound,
				dwFlags&RFE_SCREENCOORD?COORDSYS_GLOBAL:COORDSYS_PARENT);
        }
...
//
//----------------------------------------------------------------------------
void CLayout::NotifyMeasuredRange(long cpStart, long cpEnd)
{
    CNotification nf;

    Assert(cpStart>=0 && cpEnd>=0);
...
//
//----------------------------------------------------------------------------
void CLayout::NotifyTranslatedRange(
    const CSize&	size,
    long			cpStart,
    long			cpEnd)
{
...
//
//----------------------------------------------------------------------------
htmlComponent CLayout::ComponentFromPoint(long x, long y)
{
    if(_pDispNode)
    {
        CPoint pt(x,y);
        ...
        if(rcContainer.Contains(pt))
        {
            if(_pDispNode->IsScroller())
            {
                ...
                switch(part)
                {
                case CScrollbar::SBP_PREVBUTTON: return htmlComponentSbUp;
                case CScrollbar::SBP_NEXTBUTTON: return htmlComponentSbDown;
                case CScrollbar::SBP_PREVTRACK:  return htmlComponentSbPageUp;
                case CScrollbar::SBP_NEXTTRACK:  return htmlComponentSbPageDown;
                case CScrollbar::SBP_THUMB:      return htmlComponentSbVThumb;
                }
                ...
                switch(part)
                {
                case CScrollbar::SBP_PREVBUTTON: return htmlComponentSbLeft;
                case CScrollbar::SBP_NEXTBUTTON: return htmlComponentSbRight;
                case CScrollbar::SBP_PREVTRACK:  return htmlComponentSbPageLeft;
                case CScrollbar::SBP_NEXTTRACK:  return htmlComponentSbPageRight;
                case CScrollbar::SBP_THUMB:      return htmlComponentSbHThumb;
                }
            }
...
//
//------------------------------------------------------------------------------------
void CLayout::SetSelected(BOOL fSelected , BOOL fInvalidate)
{
    // select the element
    const CCharFormat* pCF = GetFirstBranch()->GetCharFormat();

    // Set the site text selected bits appropriately
    SetSiteTextSelection(fSelected, pCF->SwapSelectionColors());
...
//
//------------------------------------------------------------------------------------
void CLayout::GetParentTopLeft(CPoint* ppt)
{
    Assert(ppt);
...
//
//------------------------------------------------------------------------------------
void CLayout::GetContentRect(CRect* prc, COORDINATE_SYSTEM cs) const
{
