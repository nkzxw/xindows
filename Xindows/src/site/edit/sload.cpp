...
CSpringLoader::SpringLoad(IMarkupPointer * pmpPosition, DWORD dwMode /* =0 */)
{
    ...
    hr = CopyMarkupPointer(pMarkupServices, pmpPosition, &spmpSpringLoadFormatting);
    if (hr)
        goto Cleanup;
    ...
    if (dwMode & SL_ADJUST_FOR_INSERT_LEFT)
        AdjustPointerForInsert(spmpSpringLoadFormatting, TRUE, LEFT, LEFT);
    else if (dwMode & SL_ADJUST_FOR_INSERT_RIGHT)
        AdjustPointerForInsert(spmpSpringLoadFormatting, TRUE, RIGHT, RIGHT);
    ...
    if (dwMode & SL_TRY_COMPOSE_SETTINGS)
    {
        ...
        hr = SpringLoadComposeSettings(spmpSpringLoadFormatting);
        if (hr != S_FALSE)
            goto Cleanup;
    }
    //
    ...
    hr = pViewServices->GetCharFormatInfo(
    ...
    //hr = GetFontCommand(IDM_FONTNAME)->ConvertFormatDataToVariant(chFmtData, &varName);
    //if (hr)
    //    goto Cleanup;

    //hr = GetFontCommand(IDM_FONTSIZE)->ConvertFormatDataToVariant(chFmtData, &varSize);
    //if (hr)
    //    goto Cleanup;

    //hr = GetFontCommand(IDM_FORECOLOR)->ConvertFormatDataToVariant(chFmtData, &varColor);
    //if (hr)
    //    goto Cleanup;

    //hr = GetFontCommand(IDM_BACKCOLOR)->ConvertFormatDataToVariant(chFmtData, &varBackColor);
    //if (hr)
    //    goto Cleanup;
...
CSpringLoader::Fire(IMarkupPointer * pmpStart, IMarkupPointer * pmpEnd /*=NULL*/, BOOL fMoveCaretToStart /*=TRUE*/)
{
    ...
    if (!fHasEnd)
    {
        hr = pMarkupServices->CreateMarkupPointer(&pmpEnd);
        if (hr)
            goto Cleanup;

        hr = pmpEnd->MoveToPointer(pmpStart);
        if (hr)
            goto Cleanup;
    }
    ...
    hr = EnsureCaretPointers(fMoveCaretToStart?pmpStart:pmpEnd, &pCaret, &pmpCaret);

    if (FAILED(hr))
        goto Cleanup;

    // Remember the gravity so we can restore it at Cleanup
    hr = pmpStart->Gravity(& eGravity);
    if (hr)
        goto Cleanup;

    hr = pmpStart->SetGravity(POINTER_GRAVITY_Left);
    if (hr)
        goto Cleanup;

    hr = pmpEnd->SetGravity(POINTER_GRAVITY_Right);
    if (hr)
        goto Cleanup;
    ...
    if (V_VT(&_varSpanClass) != VT_NULL && V_VT(&_varSpanClass) != VT_EMPTY)
    {
        ...
        if (!fInSpanScope)
        {
            ...
            //hr = pInsertSpanCommand->ApplyCommandToSegment(pmpStart, pmpEnd, NULL, FALSE);
            //if (hr)
            //    goto Cleanup;

            //hr = UpdatePointerPositions(pmpStart, pmpEnd, TAGID_SPAN, TRUE);
            //if (hr)
            //    goto Cleanup;
        }
    }
    ...
    {
        //if (_fBold)
        //    hr = GetCharCommand(IDM_BOLD)->Apply(_pCommandTarget, pmpStart, pmpEnd, NULL, TRUE);
        //else
        //    hr = GetCharCommand(IDM_BOLD)->Remove(_pCommandTarget, pmpStart, pmpEnd);

        //if (hr)
        //    goto Cleanup;

        //hr = UpdatePointerPositions(pmpStart, pmpEnd, TAGID_STRONG, _fBold);
        //if (hr)
        //    goto Cleanup;
    }
    ...
    {
        //if (_fItalic)
        //    hr = GetCharCommand(IDM_ITALIC)->Apply(_pCommandTarget, pmpStart, pmpEnd, NULL, TRUE);
        //else
        //    hr = GetCharCommand(IDM_ITALIC)->Remove(_pCommandTarget, pmpStart, pmpEnd);

        //if (hr)
        //    goto Cleanup;

        //hr = UpdatePointerPositions(pmpStart, pmpEnd, TAGID_EM, _fItalic);
        //if (hr)
        //    goto Cleanup;
    }
    ...
    {
        //if (_fUnderline)
        //    hr = GetCharCommand(IDM_UNDERLINE)->Apply(_pCommandTarget, pmpStart, pmpEnd, NULL, TRUE);
        //else
        //    hr = GetCharCommand(IDM_UNDERLINE)->Remove(_pCommandTarget, pmpStart, pmpEnd);

        //if (hr)
        //    goto Cleanup;

        //hr = UpdatePointerPositions(pmpStart, pmpEnd, TAGID_U, _fUnderline);
        //if (hr)
        //    goto Cleanup;
    }
    ...
    {
        //if (_fSubscript)
        //    hr = GetCharCommand(IDM_SUBSCRIPT)->Apply(_pCommandTarget, pmpStart, pmpEnd, NULL, TRUE);
        //else
        //    hr = GetCharCommand(IDM_SUBSCRIPT)->Remove(_pCommandTarget, pmpStart, pmpEnd);

        //if (hr)
        //    goto Cleanup;

        //hr = UpdatePointerPositions(pmpStart, pmpEnd, TAGID_SUB, _fSubscript);
        //if (hr)
        //    goto Cleanup;
    }
    ...
    {
        //if (_fSuperscript)
        //    hr = GetCharCommand(IDM_SUPERSCRIPT)->Apply(_pCommandTarget, pmpStart, pmpEnd, NULL, TRUE);
        //else
        //    hr = GetCharCommand(IDM_SUPERSCRIPT)->Remove(_pCommandTarget, pmpStart, pmpEnd);

        //if (hr)
        //    goto Cleanup;

        //hr = UpdatePointerPositions(pmpStart, pmpEnd, TAGID_SUP, _fSuperscript);
        //if (hr)
        //    goto Cleanup;
    }
    ...
    if (V_VT(&_varName) != VT_NULL)
    {
        ...
        //hr = pFontCommand->ConvertFormatDataToVariant(chFmtData, &var);
        //if (hr)
        //    goto Cleanup;
        ...
        //{
        //    hr = pFontCommand->Apply(_pCommandTarget, pmpStart, pmpEnd, &_varName, TRUE);
        //    if (hr)
        //        goto Cleanup;

        //    hr = UpdatePointerPositions(pmpStart, pmpEnd, TAGID_FONT, TRUE);
        //    if (hr)
        //        goto Cleanup;
        //}
    }
    ...
    if (V_VT(&_varSize) != VT_NULL)
    {
        ...
        //if (fFontPropertyChange)
        //{
        //    hr = pFontCommand->Apply(_pCommandTarget, pmpStart, pmpEnd, &_varSize, TRUE);
        //    if (hr)
        //        goto Cleanup;
    ...
    if (V_VT(&_varColor) != VT_NULL)
    {
        ...
        //if (fFontPropertyChange)
        //{
        //    hr = pFontCommand->Apply(_pCommandTarget, pmpStart, pmpEnd, &_varColor, TRUE);
        //    if (hr)
        //        goto Cleanup;
    ...
    if (V_VT(&_varBackColor) != VT_NULL)
    {
        ...
        //if (fFontPropertyChange)
        //{
        //    hr = pFontCommand->Apply(_pCommandTarget, pmpStart, pmpEnd, &_varBackColor, TRUE);
        //    if (hr)
        //        goto Cleanup;
    ...
    if (pCaret)
    {
        hr = pmpCaret->MoveToPointer(fMoveCaretToStart?pmpStart:pmpEnd);
        if (hr)
            goto Cleanup;

        hr = pCaret->MoveCaretToPointer( pmpCaret, TRUE, TRUE, CARET_DIRECTION_INDETERMINATE ); // BUGBUG : this should be cached when you get pmpCaret
        if (hr)
            goto Cleanup;
    }
...
CSpringLoader::IsSpringLoadedAt(IMarkupPointer * pmpPosition)
{
    ...
    hr = OldCompare( _pmpPosition, pmpPosition, &iPosition);
    if (hr)
        return FALSE;
...
CSpringLoader::MarkSpringLoaded(IMarkupPointer * pmpPosition)
{
    ...
    if (pmpPosition)
    {
        hr = pMarkupServices->CreateMarkupPointer(&_pmpPosition);
        if (hr)
            goto Cleanup;

        hr = _pmpPosition->MoveToPointer(pmpPosition);
        if (hr)
            goto Cleanup;
                                    ...
                                    IMarkupPointer ** ppmpCaret )
{
    ...
    hr = pMarkupServices->CreateMarkupPointer(&pmpCaret);
    if (hr)
        goto Cleanup;

    hr = pCaret->MovePointerToCaret(pmpCaret);
    if (hr)
        goto Cleanup;

    hr = pmpCaret->IsEqualTo(pmpPosition, &fEqual);
    if (hr)
        goto Cleanup;
    ...
    hr = pmpCaret->SetGravity(POINTER_GRAVITY_Right);
                                       ...
                                       BOOL             fApplyElement )
{
    ...
    hr = GetViewServices()->RightOrSlave(pmpStart, FALSE, &mctContext, &pElemScope, NULL, NULL);
    if (hr)
        goto Cleanup;
    ...
    hr = pMarkupServices->GetElementTagId(pElemScope, &tagId);
    if (hr)
        goto Cleanup;
    ...
    if (   mctContext == (fApplyElement?CONTEXT_TYPE_EnterScope:CONTEXT_TYPE_ExitScope)
        && tagId == tagIdScope )
    {
        hr = GetViewServices()->RightOrSlave(pmpStart, TRUE, NULL, NULL, NULL, NULL);
        if (hr)
            goto Cleanup;
...
MoveEndPointer:
    ...
    hr = GetViewServices()->LeftOrSlave(pmpEnd, FALSE, &mctContext, &pElemScope, NULL, NULL);
    if (hr || !pElemScope)
        goto Cleanup;
    ...
    if (   mctContext == (fApplyElement?CONTEXT_TYPE_EnterScope:CONTEXT_TYPE_ExitScope)
        && tagId == tagIdScope )
    {
        hr = GetViewServices()->LeftOrSlave(pmpEnd, TRUE, NULL, NULL, NULL, NULL);
        if (hr)
            goto Cleanup;
    }
                            ...
                            ISegmentList    * pSegmentList )
{
    ...
    hr = CanHandleCommand(nCmdID, pSegmentList, &pmpPosition, pvarargIn);
    if (hr)
        goto Cleanup;
                                 ...
                                 VARIANTARG      * pvarargIn )
{
    ...
    if (ppmpPosition)
    {
        ...
        hr = OldCompare( pmpStart, pmpEnd, &iPosition);
        if (hr)
            goto Cleanup;
...
CSpringLoader::SpringLoadComposeSettings(IMarkupPointer * pmpNewPosition, BOOL fReset, BOOL fOutsideSpan)
{
    ...
    hr = CanSpringLoadComposeSettings(pmpNewPosition, &fSpringLoadAcrossLayout, fOutsideSpan);
...
CSpringLoader::CanSpringLoadComposeSettings(IMarkupPointer * pmpNewPosition, BOOL * pfCanOverrideSpringLoad, BOOL fOutsideSpan, BOOL fDontJump)
{
    ...
    hr = GetViewServices()->CurrentScopeOrSlave(pmpNewPosition, &pElemWalk);
    if (hr)
        goto Cleanup;

    // If we are not in a body container, don't springload.
    if (   S_OK != EdUtil::FindContainer(pMarkupServices, pElemWalk, &pElemContainer)
        || !pElemContainer
        || S_OK != pMarkupServices->GetElementTagId(pElemContainer, &tagidContainer)
    ...
    while (pElemWalk)
    {
        ...
        hr = pViewServices->IsBlockElement(pElemWalk, &fBlockElement);
        if (hr)
            goto Cleanup;
        ...
        hr = pElemWalk->get_parentElement(&pElemParent);
        if (hr)
            goto Cleanup;
...
CSpringLoader::IsCharFormatBlock(IHTMLElement * pElem)
{
    ...
    hr = pMarkupServices->GetElementTagId(pElem, &tagid);
    if (hr || !pElem)
        goto Cleanup;
...
CSpringLoader::CanJumpOverElement(IHTMLElement * pElem)
{
    ...
    {
        ELEMENT_TAG_ID tagid;
        HRESULT hr = GetMarkupServices()->GetElementTagId(pElem, &tagid);
        if (!hr && pElem && tagid == TAGID_A)
            fCanJump = TRUE;
    }

    if (!fCanJump)
        GetViewServices()->IsLayoutElement(pElem, &fCanJump);
...
CSpringLoader::InSpanScope(IMarkupPointer * pmpPosition)
{
    ...
    hr = GetViewServices()->CurrentScopeOrSlave(pmpPosition, &pElem);
    if (hr)
        goto Cleanup;
    ...
    while (tagId != TAGID_SPAN)
    {
        ...
        hr = pElem->get_parentElement(&pElemParent);
        if (hr)
            goto Cleanup;
    ...
    hr = pElem->getAttribute(_T("class"), 0, &varSpanClass);
    if (hr)
        goto Cleanup;
    ...
    if (!fInSpanScope)
    {
        hr = pElem->getAttribute(_T("className"), 0, &varSpanClass);
        if (hr)
            goto Cleanup;
...
CSpringLoader::IsSeparatedOnlyByPhraseScopes(IMarkupPointer * pmpLeft, IMarkupPointer * pmpRight)
{
    ...
    Assert(S_OK == OldCompare( pmpLeft, pmpRight, &iPosition) && iPosition == RIGHT);

    IFC(CopyMarkupPointer(pMarkupServices, pmpLeft, &spmpWalk));

    while (S_OK == OldCompare( spmpWalk, pmpRight, &iPosition) && iPosition == RIGHT)
    {
        ...
        if (   (mctContext != CONTEXT_TYPE_EnterScope && mctContext != CONTEXT_TYPE_ExitScope)
            || !spElemWalk
            || S_OK != pMarkupServices->GetElementTagId(spElemWalk, &tagId)
...
CSpringLoader::IsDocumentInHTMLEditMode()
{
    ...
    hr = pMarkupServices->QueryInterface(IID_IOleCommandTarget, (LPVOID *) &pDoc);
    if (hr || !pDoc)
        goto Cleanup;

    cmd.cmdID = IDM_HTMLEDITMODE;
    hr = pDoc->QueryStatus((GUID *)&CGID_MSHTML, 1, &cmd, NULL);
...
CSpringLoader::IsBlockEmptyForSpringLoading(IMarkupPointer * pmpStart, IHTMLElement * pElemBlock, BOOL fDontJump)
{
    ...
    hr = pMarkupServices->CreateMarkupPointer(&pmpBlockDelimiter);
    if (hr)
        goto Cleanup;

    hr = pMarkupServices->CreateMarkupPointer(&pmpWalk);
    if (hr)
        goto Cleanup;
    ...
    {
        hr = pmpWalk->MoveToPointer(pmpStart);
        if (hr)
            goto Cleanup;

        if (iDirection == -1)
            hr = pmpBlockDelimiter->MoveAdjacentToElement(pElemBlock, ELEM_ADJ_AfterBegin);
        else
            hr = pmpBlockDelimiter->MoveAdjacentToElement(pElemBlock, ELEM_ADJ_BeforeEnd);
        if (hr)
            goto Cleanup;

        // Search left or right.
        while (S_OK == (OldCompare(pmpWalk, pmpBlockDelimiter, &iResult) && (iResult*iDirection) < 0))
        {
            ...
            cch = 1;
            if (iDirection == -1)
                hr = pViewServices->LeftOrSlave(pmpWalk, TRUE, &mctContext, &pElemWalk, &cch, &ch);
            else
                hr = pViewServices->RightOrSlave(pmpWalk, TRUE, &mctContext, &pElemWalk, &cch, &ch);
            if (hr)
                goto Cleanup;
            ...
            case CONTEXT_TYPE_EnterScope:
                ...
                if (!fDontJump && pElemWalk && CanJumpOverElement(pElemWalk))
                    pmpWalk->MoveAdjacentToElement(pElemWalk, ((iDirection==-1)?ELEM_ADJ_BeforeBegin:ELEM_ADJ_AfterEnd));
                break;

            case CONTEXT_TYPE_NoScope:
                // BRs mean we're done looking in that direction.
                if (pElemWalk && S_OK == pMarkupServices->GetElementTagId(pElemWalk, &tagId) && TAGID_BR == tagId)
                    fStopLooking = TRUE;
                break;
...
CSpringLoader::SpringLoadFormatsAcrossLayout(IMarkupPointer * pmpPosition, IHTMLElement * pElemBlock, BOOL fActuallySpringLoad)
{
    ...
    cch = 1;
    if (   S_OK != pViewServices->LeftOrSlave(pmpPosition, FALSE, &mctContext, &pElemToLeft, &cch, &ch)
        || (mctContext == CONTEXT_TYPE_Text && cch == 1 && !IsWhiteSpace(ch) && WCH_NBSP != ch))
        goto Cleanup;
    cch = 1;
    if (   S_OK != pViewServices->RightOrSlave(pmpPosition, FALSE, &mctContext, &pElemToRight, &cch, &ch)
        || (mctContext == CONTEXT_TYPE_Text && cch == 1 && !IsWhiteSpace(ch) && WCH_NBSP != ch))
        goto Cleanup;
    ...
    {
        hr = pmpWalk->MoveToPointer(pmpPosition);
        if (hr)
            goto Cleanup;
        ...
        cch = 1;
        if (   S_OK == AdjustPointerForInsert(pmpWalk, TRUE, LEFT, LEFT)
            && S_OK == pViewServices->LeftOrSlave(pmpWalk, FALSE, &mctContext, NULL, &cch, &ch)
            && mctContext == CONTEXT_TYPE_Text && cch == 1 && !IsWhiteSpace(ch) && WCH_NBSP != ch)
        {
    ...
    if (!pElemToRight || !CanJumpOverElement(pElemToRight))
    {
        ...
        cch = 1;
        if (   S_OK == AdjustPointerForInsert(pmpWalk, TRUE, RIGHT, RIGHT)
            && S_OK == pViewServices->RightOrSlave(pmpWalk, FALSE, &mctContext, NULL, &cch, &ch)
            && mctContext == CONTEXT_TYPE_Text && cch == 1 && !IsWhiteSpace(ch) && WCH_NBSP != ch)
        {
    ...
    for (iDirection = -1 ; iDirection <= 1 ; iDirection += 2)
    {
        ...
        while (S_OK == OldCompare(pmpWalk, pmpBlockDelimiter, &iResult) && (iResult*iDirection) < 0)
        {
            ...
            case CONTEXT_TYPE_EnterScope:
                ...
                {
                    hr = pmpWalk->MoveAdjacentToElement(pElemWalk, ((iDirection==-1)?ELEM_ADJ_BeforeBegin:ELEM_ADJ_AfterEnd));
                    if (hr)
                        goto Cleanup;

                    hr = OldCompare(pmpWalk, pmpBlockDelimiter, &iResult);
                    if (hr)
                        goto Cleanup;
            ...
            case CONTEXT_TYPE_NoScope:
                if (   pElemWalk
                    && !pmpAcrossLayout)
                {
                    hr = pMarkupServices->CreateMarkupPointer(&pmpAcrossLayout);
                    if (hr)
                        goto Cleanup;
                }
                break;
            ...
            if (pmpAcrossLayout)
            {
                ...
                hr = pmpAcrossLayout->MoveToPointer(pmpWalk);
                if (hr)
                    goto Cleanup;
            }
        }
...
SpringLoad:
    ...
    if (dwSpringLoad && fActuallySpringLoad)
    {
        ...
        hr = SpringLoad(pmpAcrossLayout, 0);
        if (hr)
            goto Cleanup;
...
CSpringLoader::AdjustPointerForInsert(IMarkupPointer * pWhereIThinkIAm, BOOL fNotAtBOL, INT inBlockcDir, INT inTextDir)
{
    ...
    CXindowsEditor * pEd = _pCommandTarget->GetEditor();
    ...
    hr = pEd->AdjustPointer( pWhereIThinkIAm, fNotAtBOL, inBlockcDir, inTextDir, spLeftEdge, spRightEdge, ADJPTROPT_AdjustIntoURL);
    if( hr == E_HITBOUNDARY )
        hr = S_OK;
