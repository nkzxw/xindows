...
// LPARAM helpers
//
BOOL HaveCompositionString(LONG lparam) { return ( 0 != (lparam & (GCS_COMPSTR | GCS_COMPATTR))); }
BOOL CleanupCompositionString(LONG lparam) { return ( 0 == lparam ); }
BOOL HaveResultString(LONG lparam) { return (0 != (lparam & GCS_RESULTSTR)); }
...
{
    HWND hwnd;
    HRESULT hr = GetEditor()->GetViewServices()->GetViewHWND( &hwnd);
    Assert( hwnd );
...
CIme::SetCaretVisible( BOOL fVisible )
{
    ...
    {
        hr = SetCaretVisible( _pManager->GetDoc(), _fCaretVisible = fVisible );
    }
    else
    {
        hr = S_OK;
    ...
    else if ( IsImeEditable() )
    {
        ...
        if (peTrackerNotify && _pActiveTracker)
        {
            ...
            else if (_pActiveTracker->GetSelectionType() == SELECTION_TYPE_Caret)
            {
                ...
                hr = ime.CheckInsertResultString( lparam );
            }
        }
    }
    else
...
{
    HRESULT hr = _pSelRenSvc->ClearSegments( TRUE );
    ...
    HIGHLIGHT_TYPE HighlightType )
{
    ...
    hr = _pManager->GetDoc()->QueryInterface(IID_IMarkupServices, (void**) & pMarkup );
    if (hr)
        goto Cleanup;

    hr = epStart->MoveToPointer( _pmpStartUncommitted );
    ...
    {
        hr = epStart.Scan(RIGHT, dwSearch, &dwFound);
        if (hr)
            goto Cleanup;
    ...
    hr = epEnd->MoveToPointer( epStart );
    if (hr)
        goto Cleanup;
    ...
    hr = _pSelRenSvc->AddSegment( epStart, epEnd, HighlightType, &iSegment);
    if (hr)
        goto Cleanup;
...
CIme::SetCompositionForm()
{
    if ( IME_LEVEL_2 == GetIMELevel() )
    {
        ...
        if ( hIMC )
        {
            COMPOSITIONFORM cf;
            long lLineHeight;
            HRESULT hr = GetCompositionPos( &cf.ptCurrentPos, &cf.rcArea, &lLineHeight );
...
{
    /*if (pManager)
        _undoUnit.Begin(IDS_EDUNDOTYPING); wlw note*/
    ...
    const LPARAM lparam )
{
    ...
    if ( CleanupCompositionString(lparam) || HaveResultString(lparam) )      // If result string..
    {
        ...
        cch = (LONG)GetCompositionStringInfo( GCS_RESULTSTR,
                                              achCompStr, ARRAYSIZE(achCompStr),
                                              NULL, 0, NULL, NULL );

        if (( achCompStr[0] == 32) && (cch == 1))
        {
            hr = pCaretTracker->HandleSpace( achCompStr[0] );            
        }
        else
        {
        ...
        hr = ReplaceRange(achCompStr, cch, TRUE, -1, TRUE);
        if (FAILED(hr))
            goto Cleanup;
    ...
    TRACKER_NOTIFY * peTrackerNotify )
{
    ...
    if (IsIMEComposition())
    {
        hr = _pIme->IMENotify(wparam,lparam);
    }
...
HIGHLIGHT_TYPE HighlightTypeFromAttr( BYTE a )
{
    ...
    TRACKER_NOTIFY * peTrackerNotify )
{
    ...
    if (  CleanupCompositionString(lparam) || HaveResultString(lparam)  ) // Any final readings?
    {
        ...
        hr = CheckInsertResultString( lparam );
        if (hr)
        {
            if (hr == S_FALSE)
            {
    ...
    if ( HaveCompositionString(lparam) )            // In composition mode?
    {
        ...
        else
        {
            ...
            ReplaceRange( achCompStr, cchCompStr, fShowCaret, ichCursor);
            ...
            if ( cchCompStr && cchCompStr <= cchAttrib && !_fKorean )     // no Korean style needed
            {
                ...
                for (ichMost = 1; ichMost < cchCompStr; ichMost++)
                {
                    ...
                    hr = AddHighlightSegment( iHighlightSegment++, FALSE, ichMin, ichMost, HighlightTypeFromAttr(abCur) );
                    if (hr)
                        goto Cleanup;
                ...
                if (ichMin != ichMost)
                {
                    hr = AddHighlightSegment( iHighlightSegment, TRUE, ichMin, ichMost, HighlightTypeFromAttr(abCur) );
                    if (hr)
                        goto Cleanup;
                }

                /*if (IsTagEnabled(tagEdIMEAttr)) wlw note*/
                {
            ...
            fShowCursor = FALSE;
            {
                ...
                if ( _fKorean && cchCompStr )
                {
                    ...
                    hr = AddHighlightSegment( 0, FALSE, 0, cchCompStr, HIGHLIGHT_TYPE_ImeHangul );
                    if (hr)
                        goto Cleanup;
                }
                else
    ...
    if ( IMN_OPENCANDIDATE == wparam || IMN_CLOSECANDIDATE == wparam  )
    {
        ...
        if (hIMC)
        {
            ...
            if ( IMN_OPENCANDIDATE == wparam && !_fKorean )  // Set candidate to caret.
            {
                ...
                RECT rc;
                long lLineHeight;
                HRESULT hr = GetCompositionPos(&ptCaret, &rc, &lLineHeight);
