...
HRESULT CBlockDirCommand::AdjustElementMargin(IHTMLElement* pCurElement)
{
    ...
    GetMarkupServices()->GetElementTagId(pCurElement, &tagId);
...
CBlockDirCommand::SetElementAlignment(IHTMLElement *pElement, BSTR szAlign)
{
    ...
    hr = FindAlignment(pParent, &szDir);
    if (FAILED(hr))
        goto Cleanup;        

    GetMarkupServices()->GetElementTagId(pParent, &tagId);
    ...
    if (FAILED(hr))
        ...
        hr = AdjustElementMargin(pElement);
    ...
    BSTR             *pszDir)
{
    ...
    *pszDir = NULL;
    ...
    do
    {
        hr = GetElementAlignment(pCurrent, pszDir);
        if (SUCCEEDED(hr))
            break; // done
    
        ReplaceInterface(&pOldElement, pCurrent);
        ClearInterface(&pCurrent);
        hr = pOldElement->get_parentElement(&pCurrent);
        if (FAILED(hr))
            goto Cleanup;        
    }
    while (pCurrent);
       ...
       CGetBlockFmtCommand::_blockFmts[] = 
{
    ...
    { TAGID_NULL,    0/*IDS_BLOCKFMT_NORMAL wlw note*/ }, // depends on the current default tag 
...
{
    { TAGID_PRE,     0/*IDS_BLOCKFMT_PRE_TAG wlw note*/ },
...
HRESULT CGetBlockFmtCommand::PrivateQueryStatus(OLECMD* pCmd, OLECMDTEXT* pcmdtext)
{
...
CGetBlockFmtCommand::GetDefaultBlockTag( IMarkupServices *pMarkupServices, ELEMENT_TAG_ID *ptagId)
{
    ...
    *ptagId = TAGID_P; // default for most cases
    ...
    hr = pMarkupServices->QueryInterface(IID_IOleCommandTarget, (LPVOID *)&pCommandTarget);
    if (FAILED(hr))
        goto Cleanup;

        
    hr = pCommandTarget->Exec((GUID *)&CGID_MSHTML,
                                  IDM_DEFAULTBLOCK,
                                    ...
                                    ELEMENT_TAG_ID      *ptagId )
{
    ...
    // Check standard case
    for (i = 0; i < ARRAYSIZE(_blockFmts); i++)
    {
    ...
    // Check <tagId> case
    for (i = 0; i < ARRAYSIZE(_tagBlockFmts); i++)
    {
...
{  
    ...
    if (pvarargOut)
    {
        sabound.cElements = ARRAYSIZE(_blockFmts);
        sabound.lLbound = 0;
...
CGetBlockFmtCommand::Init()
{
    ...
    for (int i = 0; i < ARRAYSIZE(_blockFmts); i++)
...
CGetBlockFmtCommand::LoadDisplayNames(HINSTANCE hinst)
{
    ...
    Assert(_fLoaded);
    hr = LoadDisplayNamesHelper(hinst);
...
CGetBlockFmtCommand::LoadDisplayNamesHelper(HINSTANCE hinst)
{
    ...
    for (i = 0; i < ARRAYSIZE(_blockFmts); i++)
    {
        if (!LoadString(hinst, _blockFmts[i]._idsName, szBuffer, ARRAYSIZE(szBuffer)))
        {
            hr = E_FAIL;
            goto Cleanup;
        }
...
CGetBlockFmtCommand::LoadStringTable(HINSTANCE hinst)
{
    ...
    for (i = 0; i < ARRAYSIZE(_tagBlockFmts); i++)
    {
        if (!LoadString(_afxGlobalData._hInstResource/*g_hInstance wlw note*/, _tagBlockFmts[i]._idsName, szBuffer, ARRAYSIZE(szBuffer)))
        {
            hr = E_FAIL;
            goto Cleanup;
        }
...
CBlockPointer::CBlockPointer(CXindowsEditor *pEd)
{
    _pEd = pEd;
    _type = NT_Undefined;
    _pElement = NULL;
...
{  
    ...
    /*IFR( undoUnit.Begin(IDS_EDUNDOGENERICTEXT) ); wlw note*/
    for (;;)
    {
        ...
        CreateCaretMarker(&spCaretMarker);
        IFR( ApplyListCommand(pStart, pEnd, FALSE, TRUE) );
        RestoreCaret(spCaretMarker);
    }
...
{  
    ...
    {        
        ...
        else
        {
            ...
            /*IFC( undoUnit.Begin(IDS_EDUNDOGENERICTEXT) ); wlw note*/
            for (;;)
            {
    ...
    if (pvarargOut)
    {
        ...
        hr = GetSegmentListBlockFormat(spSegmentList, &tagId);
        if (FAILED(hr))
        {
...
{   
    ...
    *ptagIdOut = TAGID_NULL;
    ...
    hr = FindCommonElement(GetMarkupServices(), GetViewServices(), pStart, pEnd, &spElement, TRUE);
...
CBlockFmtCommand::ApplyComposeSettings(IMarkupPointer *pStart, IMarkupPointer *pEnd)
{
    ...
    if (psl)
    {
        psl->SpringLoadComposeSettings(NULL, TRUE);

        if (pStart->IsEqualTo(pEnd, &fEqual) || !fEqual)
        {
            psl->Fire(pStart, pEnd);
        }
    }
...
CBlockFmtCommand::ApplyCommand(IMarkupPointer *pStart, IMarkupPointer *pEnd)
{
    ...
    do
    {
        ...
        if (bpCurrent.IsLeafNode() || bpCurrent.GetType() == NT_FlowLayout)
        {
            ...
            if ( ShouldRemoveFormatting(_tagId))
            {
                /*IFR( DYNCAST(CRemoveFormatCommand, GetEditor()->GetCommandTable()->Get(IDM_REMOVEFORMAT))->Apply(spStart, spEnd) ); wlw note*/
            }
            else
            {
