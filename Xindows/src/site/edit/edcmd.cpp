                          ...
                          IHTMLViewServices ** ppViewServices)
{
    ...
    hr = pMarkupServices->QueryInterface(IID_IHTMLDocument2, (LPVOID *)&pDoc);
    if (FAILED(hr)) {
        *ppViewServices = NULL;
        return hr;
    }

    hr = pDoc->QueryInterface(IID_IHTMLViewServices, (LPVOID *)ppViewServices);
    ...
    CMshtmlEd *              pTarget  )
{
    ...
    _pcmdtgt = pTarget;
    hr = PrivateExec( nCmdexecopt, pvarargIn, pvarargOut );
    _pcmdtgt = NULL;
    ...
    CMshtmlEd *              pTarget  )
{
    HRESULT hr;
    _pcmdtgt = pTarget;
    hr = PrivateQueryStatus( rgCmds, pcmdtext );
    _pcmdtgt = NULL;
    RRETURN( hr );
}    
...
{ 
    ...
    hr = _pcmdtgt->GetSegmentList( ppSegmentList);
...
CCommand::CanSplitBlock( IMarkupServices *pMarkupServices , IHTMLElement* pElement )
{
    ...
    pMarkupServices->GetElementTagId( pElement, &curTag );
                                    ...
                                    BOOL            fOuter)
{
    ...
    *ppElement = NULL;
    ...
    if (fOuter)
    {
        if (FAILED(pViewServices->LeftOrSlave(pStart, FALSE, &context, ppElement, NULL, NULL)))
            goto Cleanup;
    ...
    {
        if (FAILED(pViewServices->RightOrSlave(pStart, FALSE, &context, ppElement, NULL, NULL)))
            goto Cleanup;
    ...
    if (fOuter)
    {
        if (FAILED(pViewServices->RightOrSlave(pEnd, FALSE, &context, &pElementRHS, NULL, NULL)))
            goto Cleanup;
    }
    else
    {
        if (FAILED(pViewServices->LeftOrSlave(pEnd, FALSE, &context, &pElementRHS, NULL, NULL)))
            goto Cleanup;
    }
    ...
    if (FAILED(pElementRHS->QueryInterface(IID_IObjectIdentity, (LPVOID *)&pObjectIdent)))
        goto Cleanup; // fail

    hr = pObjectIdent->IsEqualObject(*ppElement);
                    ...
                    IHTMLElement** ppElementNew )
{
    ...
    switch ( inElemInfluence )
    {
        ...
        case elemInfluenceCompleteContain:
        {
            ...
            hr = pMarkupServices->RemoveElement( pElement );
            if (!hr) hr = pStartPointer->IsEqualTo(pStart, &bEqual);
            if (hr) goto Cleanup;
            ...
            hr = pEndPointer->IsEqualTo(pEnd, &bEqual);
            if (hr) goto Cleanup;
        ...
        case elemInfluenceOverlapWithin:
        {
            ...
            if (!hr) hr = pEndPointer->IsEqualTo(pEnd, &bEqual);
            if (hr) goto Cleanup;
        ...
        case elemInfluenceOverlapOutside:
        {
            ...
            hr = pMarkupServices->RemoveElement( pElement );
            if (!hr) hr = pStart->IsEqualTo(pStartPointer, &bEqual);
            if (hr) goto Cleanup;
                                ...
                                IMarkupPointer  **ppEnd)
{
    ...
    hr = pDoc->get_activeElement(&pElement);
    if (FAILED(hr))
        goto Cleanup;
    ...
    hr = pStart->MoveAdjacentToElement( pElement, ELEM_ADJ_AfterBegin );
    if (FAILED(hr))
        goto Cleanup;
        
    hr = pEnd->MoveAdjacentToElement( pElement, ELEM_ADJ_BeforeEnd );
                                 ...
                                 MARKUP_CONTEXT_TYPE *pContext)
{
    ...
    hr = CopyMarkupPointer(pMarkupServices, pMarkupPointer, &pCurrent);
    if (FAILED(hr))
        goto Cleanup;

    for (;;) {
        hr = pViewServices->LeftOrSlave(pCurrent, TRUE, &context, &pElement, NULL, NULL);
        if (FAILED(hr))
            break; // not found
        ...
        hr = pMarkupServices->GetElementTagId(pElement, &tagIdCurrent);
        if (FAILED(hr))
            goto Cleanup;
    ...
    MARKUP_CONTEXT_TYPE *pContext )
{
    ...
    for (;;) {
        hr = pViewServices->RightOrSlave(pCurrent, TRUE, &context, &pElement, NULL, NULL);
        if (FAILED(hr))
            break; // not found
                                 ...
                                 IHTMLElement    **ppNewElement )
{
    ...
    hr = pSegmentEnd->SetGravity( POINTER_GRAVITY_Right);
    if ( FAILED(hr))
        goto Cleanup;
    hr = pTagEnd->SetGravity( POINTER_GRAVITY_Right);
    if ( FAILED(hr))
        goto Cleanup;      
    //
    ...
    hr = InsertElement(pMarkupServices, pElement, pTagStart, pSegmentEnd);
    if (FAILED(hr))
        goto Cleanup;
