...
#define UNICODE_SUBRANGES   ARRAYSIZE(s_asidUnicodeSubRangeScriptMapping)
...
//
//-----------------------------------------------------------------------------
CODEPAGE DefaultCodePageFromScript(/*[in, out]*/SCRIPT_ID* psid, /*[in]*/CODEPAGE cpDoc, /*[in]*/LCID lcid)
{
...
BYTE DefaultCharSetFromScriptAndCharset(SCRIPT_ID sid, BYTE bCharSetCF)
{
    ...
    if(bCharSet == SPECIAL_CHARSET)
    {
        ...
        {
            bCharSet = bCharSetCF==JOHAB_CHARSET ? JOHAB_CHARSET : HANGUL_CHARSET;
        }
        else
        {
...
BYTE DefaultCharSetFromScriptAndCodePage(SCRIPT_ID sid, UINT uiFamilyCodePage)
{
    ...
    if(bCharSet == SPECIAL_CHARSET)
    {
        bCharSet = (uiFamilyCodePage>=1250 && uiFamilyCodePage<=1258)
            ? s_ab125xCharSets[uiFamilyCodePage-1250] : ANSI_CHARSET;
    }
    ...
    default:
        {
            ...
            if(OK(hr) && g_pMultiLanguage!=NULL)
            {
                ...
                hr = g_pMultiLanguage->QueryInterface(IID_IMLangFontLink2, (void**)&pMLangFontLink2);

                if(OK(hr))
                {
                    hr = pMLangFontLink2->CodePageToScriptID(uiFamilyCodePage, &sid);
                }
...
#ifndef NO_UTF16
    Assert(sid==sidHalfWidthKana || sid==sidSurrogateA
        || sid==sidSurrogateB || sid<sidLim);
#else
    Assert(sid==sidHalfWidthKana || sid<sidLim);
#endif

    // Make sure our sid order hasn't changed.
    Assert(sidDefault<sidMerge && sidMerge<sidAsciiSym
        && sidAsciiSym<sidAsciiLatin && sidAsciiLatin<sidLatin);

    return (sid<=sidLatin)?sidAsciiLatin:((sid==sidHalfWidthKana)?sidKana:sid);
}
