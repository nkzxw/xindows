...
DWORD GetLangBits(IMLangFontLink* pMLangFontLink, WCHAR wc)
{
    ...
    else if(pMLangFontLink)
    {
        ...
        {
            pMLangFontLink->GetCharCodePages(wc, &dwCodePages);
        }
        else
        {
...
DWORD GetFontScriptBits(HDC hDC, const TCHAR* szFaceName, LOGFONT* plf)
{
        ...
        PRIMARYLANGID(LANGIDFROMLCID(_afxGlobalData._lcidUserDefault))==LANG_ENGLISH)
    {
        ...
        SUCCEEDED(hr=g_pMultiLanguage->QueryInterface(IID_IMLangFontLink, (void**)&pMLangFontLink)))
    {
        LOGFONT lf;
        HFONT hFont;

        lf = *plf;
        ::_tcsncpy(lf.lfFaceName, szFaceName, ARRAYSIZE(lf.lfFaceName));
...
BYTE CharSetFromLangBits(DWORD dwLangBits)
{
                    ...
                    CCharFormat*    pcf)            // OUT
{
    ...
    {
        fNewlyFetchedFromRegistry = SelectScriptAppropriateFont(
            (dwScriptBits&SBITS_SURROGATE_A)?sidSurrogateA:sidSurrogateB,
            DEFAULT_CHARSET,
            pDoc,
            pcf);
        ...
        if(hSrcFont && pMLangFontLink)
        {
            ...
            if(::GetLocaleInfo(lcidLang, LOCALE_IDEFAULTANSICODEPAGE, pszCodePage, ARRAYSIZE(pszCodePage)) &&
                SUCCEEDED(pMLangFontLink->CodePageToCodePages(StrToInt(pszCodePage), &dwCodePages)))
            {
                dwCodePages &= dwScriptBits;
                if(dwCodePages && dwCodePages!=dwScriptBits &&
              ...
              CStackDataAry<CCcs*, FLTO_CACHE_SIZE>* paryLinkFonts)
{
    ...
    if(dwLangBits & (SBITS_SURROGATE_A|SBITS_SURROGATE_B))
    {
        ...
        if(fNewlyFetchedFromRegistry)
        {
            ForceScriptIdOnUserSpecifiedFont(
                pdci->_pDoc->_pOptionSettings,
                (dwLangBits&SBITS_SURROGATE_A)?sidSurrogateA:sidSurrogateB);
        }
    ...
    if((pccs->GetBaseCcs()->_dwLangBits&dwLangBits)==0 || (ppccs = paryLinkFonts->Append())==NULL)
    {
        pccs->Release();
        return pccsDefault;
    }
...
int FontLinkTextOut(
                    ...
                    UINT uMode)
{
    CStackDataAry<CCcs*, FLTO_CACHE_SIZE> aryLinkFonts;
    CCcs* pccsRun;
    CCcs* pccsNextRun;
    CCcs* pccsDefault;
    LPCTSTR pPastRun;
    ...
    int iDx;
    int i;
    CStackDataAry<TCHAR, CHUNKSIZE> aryStringCopy;
    ...
    if((pccsDefault==NULL) || (pccsDefault->GetBaseCcs()->_dwLangBits==0))
    {
        if(pccsDefault != NULL)
        {
            pccsDefault->Release();
    ...
    if(g_pMultiLanguage)
    {
        g_pMultiLanguage->QueryInterface(IID_IMLangFontLink,(void**)&pMLangFontLink);
    }
                    ...
                    int* piDx)
{
    ...
    if((fGlyph || fRTL) && !FALSE/*g_fExtTextOutGlyphCrash wlw note*/)
    {
        HRESULT hr;
...
//
//+---------------------------------------------------------------
void DrawUnderlineStrikeOut(int x, int y, int iLength, HDC hDC, HFONT hFont, const RECT* prc)
{
    int iOldBkMode;
    HFONT hOldFont;
...
//
//----------------------------------------------------------------
BOOL NeedsFontLinking(HDC hdc, CCcs* pccs, LPCTSTR pString, int cch, CDocument* pDoc)
{
    CBaseCcs* pBaseCcs;
    IMLangFontLink* pMLangFontLink = NULL;

    if(pccs==NULL || pccs->GetBaseCcs()==NULL ||
        pString==NULL || cch <= 0)
    {
        return FALSE;
    }
...
BOOL SelectScriptAppropriateFont(
                                 SCRIPT_ID sid,     // IN
                                 BYTE bCharSet,     // IN
                                 CDocument* pDoc,   // IN
...
BOOL ScriptAppropriateFaceNameAtom(
                              SCRIPT_ID sid,        // IN
                              CDocument* pDoc,      // IN
                              BOOL fFixed,          // IN
                              LONG* platmFontFace)  // OUT                              
{
    ...
    if(fFetch)
    {
        ...
        latmFontFace = fFixed ? CS.latmFixedFontFace : CS.latmPropFontFace;
    }
