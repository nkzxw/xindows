...
CRecalcLinePtr::CRecalcLinePtr(CDisplay* pdp, CCalcInfo* pci)
{
    ...
    _cLeftAlignedLayouts = _cRightAlignedLayouts = 0;
    _fIsEditable = pFlowLayout->IsEditable();
    
    if(pElementFL->Tag()==ETAG_MARQUEE && !_fIsEditable)
    {
...
//
//-------------------------------------------------------------------------
void CRecalcLinePtr::ApplyLineIndents(CTreeNode* pNode, CLine* pLineMeasured, UINT uiFlags)
{
    ...
    if(_iPF!=iPF || _fInnerPF!=fInner)
    {
        ...
        if((pPF->_cuvLeftIndentPercent.IsNull()?FALSE:pPF->_cuvLeftIndentPercent.GetUnitValue())
            || (pPF->_cuvRightIndentPercent.IsNull()?FALSE:pPF->_cuvRightIndentPercent.GetUnitValue()))
        {
...
//
//-------------------------------------------------------------------------
CTreeNode* CRecalcLinePtr::CalcInterParaSpace(CLSMeasurer* pMe, LONG iPrevLine, BOOL fFirstLineInLayout)
{
    LONG iLine;
 ...
 *
 */
LONG CRecalcLinePtr::NetscapeBottomMargin(CLSMeasurer* pMe)
{
    ...
    LONG yBeforeSpace)
{
    ...
    // that must be used.
    for(pLine=First(iAt); pLine&&At()<=iLineFirst; pLine=Next())
    {
        // update y to skip by 
        if(At() == iLineFirst)
        {
                    ...
                    && pFF->_fAlignedLayout)
                {
                    ...
                    for(;;)
                    {
                            ...
                            xWidthSite>xWidthMax)
                        {
                            ...
                            // add a clear line
                            ClearObjects(pLineNew, iLineStart, iliClear, &yHeight);
                            ...
                            RecalcMargins(iLineStart,
                                fMeasuringSitesAtBOL?iLineFirst+iClearLines:Count()-1,
                                yHeight, 0);
                    ...
                    if(pxMaxLineWidth)
                    {
                        if(!fRTLDisplay)
                        {
                            *pxMaxLineWidth = max(*pxMaxLineWidth,
                                pLineNew->_xLeftMargin+pLineNew->_xLineWidth);
                        }
                        else
                        {
                            *pxMaxLineWidth = max(*pxMaxLineWidth,
                                pLineNew->_xRightMargin+pLineNew->_xLineWidth);
                        }
                    }
    ...
    LONG*   pyHeight)
{
    ...
    if(pLine->_cch>0 && yAt<=(*pyHeight+pLine->_yHeight))
    {
        // nothing to clear
    ...
    else if(pLine->_cch || pLine->IsFrame() && !_marginInfo._fAutoClear)
    {
        if(yAt > *pyHeight)
        {
            ...
            if(pLineNew)
            {
                ...
                pLineNew->_xLeftMargin = _marginInfo._xLeftMargin;
                pLineNew->_xRightMargin = _marginInfo._xRightMargin;
                pLineNew->_xLineWidth = max (0L,
                    (_marginInfo._xLeftMargin?_xMarqueeWidth:0)+
...
//
//-----------------------------------------------------------------------------
BOOL CRecalcLinePtr::MeasureLine(
                                 CLSMeasurer& me,
                                 UINT   uiFlags,
                                 INT*   piLine,
                                 LONG*  pyHeight,
                                 ...
                                 LONG*  pxMaxLineWidth)
{
    ...
    if(!CheckForClear())
    {
        ...
        if(!fClearMarginsRequired)
        {
            ...
            if(!_marginInfo._fAutoClear || me._li._cch>0)
            {
                ...
                me._li._yHeight  += _yBordTop + _yBordBottom + _yPadTop  + _yPadBottom;
                me._li._yDescent += _yBordBottom + _yPadBottom;
                me._li._yHeight  += (LONG)me._li._yBeforeSpace;
                me._li._yExtent  += _yBordTop + _yBordBottom + _yPadTop  + _yPadBottom;
    ...
    if(!_marginInfo._fAutoClear || me._li._cch>0)
    {
        ...
        pliNew->_xLineWidth = pliNew->_xLeft +
            pliNew->_xWidth + pliNew->_xLineOverhang + pliNew->_xRight;
        ...
        {
            *pxMaxLineWidth = max(*pxMaxLineWidth,
                (!pliNew->_fRTL?pliNew->_xLeftMargin:pliNew->_xRightMargin)+
                pliNew->_xWidth+
                pliNew->_xLeft+
...
//
//-----------------------------------------------------------------------------
LONG CRecalcLinePtr::CalcAlignedSitesAtBOL(
      CLSMeasurer* pme,
      CLine*    pLineMeasured,
      UINT      uiFlags,
      INT*      piLine,
      ...
      BOOL*     pfClearMarginsRequired)
{
    ...
    if(ptp->IsText() && ptp->Cch())
    {
        ...
        if(!IsWhite(ch) ||
            pNode->GetParaFormat()->HasInclEOLWhite(SameScope(pNode, pme->_pFlowLayout->ElementContent())))
        {
    ...
    return CalcAlignedSitesAtBOLCore(pme, pLineMeasured, uiFlags, piLine,
        pyHeight, pxWidth, pyAlignDescent, pxMaxLineWidth, pfClearMarginsRequired);
}

LONG CRecalcLinePtr::CalcAlignedSitesAtBOLCore(
        CLSMeasurer* pme,
        CLine*      pLineMeasured,
        UINT        uiFlags,
        INT*        piLine,
        ...
        BOOL*       pfClearMarginsRequired)
{
    ...
    *pfClearMarginsRequired = FALSE;
    ...
    for(;;)
    {
        ...
        if(!_fIsEditable && ptp->IsBeginElementScope() && pCF->IsDisplayNone())
        {
            cp += pme->GetNestedElementCch(pNode->Element(), &ptp) - 1;
        }
        else if(ptp->IsBeginElementScope() && pNode->NeedsLayout())
        {
...
//
//-----------------------------------------------------------------------------
BOOL CRecalcLinePtr::CheckForClear(CTreeNode* pNode)
{
    BOOL fClearLeft;
    BOOL fClearRight;
...
//-----------------------------------------------------------------------------
void CRecalcLinePtr::FixupChunks(CLSMeasurer& me, INT* piLine)
{
...
//
//-----------------------------------------------------------------------------
CTreeNode* CRecalcLinePtr::CalcParagraphSpacing(CLSMeasurer* pMe, BOOL fFirstLineInLayout)
{
...
//
//-----------------------------------------------------------------------------
void CRecalcLinePtr::SetupMeasurerForBeforeSpace(CLSMeasurer* pMe)
{
    CFlowLayout*    pFlowLayout = _pdp->GetFlowLayout();
    CElement*       pElementFL  = pFlowLayout->ElementContent();
    ...
    if(ptp!=pMe->GetPtp() || pMe->_pLS->_pNodeForAfterSpace)
    {
        pMe->SetPtp(ptp, -1);
    ...
    if(DifferentScope(pNode, pElementFL))
    {
        ...
        pNodeTemp = pMe->GetPtp()->IsBeginElementScope() ? pNode->Parent() : pNode;
        ...
        while(DifferentScope(pNodeTemp, pElementFL))
        {
                ...
                && pFlowLayout->IsElementBlockInContext(pElement))
            {
                ...
                _xPadLeftPerLine += pFF->_cuvPaddingLeft.XGetPixelValue(
                    _pci, _pci->_sizeParent.cx, lFontHeight);
...
//
//-----------------------------------------------------------------------------
void CRecalcLinePtr::CalcAfterSpace(CLSMeasurer* pMe, LONG cpMax)
{
    CFlowLayout* pFlowLayout = _pdp->GetFlowLayout();
    CTreeNode*  pNode;
    CElement*   pElement;
    ...
    for(cpCurrent=pMe->GetCp(),ptp=pMe->GetPtp(); cpCurrent<cpMax; ptp=ptp->NextTreePos())
    {
        if(ptp->IsPointer())
        {
            continue;
...
//
//-----------------------------------------------------------------------------
CTreePos* CRecalcLinePtr::CalcBeforeSpace(CLSMeasurer* pMe, BOOL fFirstLineInLayout)
{
            ...
            if(ptp->IsEndElementScope())
            {
                ...
                if(pFlowLayout->IsElementBlockInContext(pElement))
                {
                    ...
                    if(pElement->_fBreakOnEmpty
                        && (fSeenBeginBlockTag
                        || (pMe->_fLastWasBreak && _fIsEditable)))
                    {
                        break;
                    }
                    ...
                    if(pElement->HasFlag(TAGDESC_LISTITEM)
                        && (pMe->_li._fHasBulletOrNum || pMe->_fLastWasBreak))
                    {
                        break;
                    }
            ...
            else if(ptp->IsBeginElementScope())
            {
                ...
                else
                {
                        ...
                        && pElement->IsInlinedElement())
                    {
                        ...
                        else if(pElement->Tag()==ETAG_LI&& !pNode->IsAbsolute())
                        {
                            Assert(pElement->IsBlockElement());
                        ...
                        if(pElement->Tag()==ETAG_DD && pPF->_fFirstLineIndentForDD)
                        {
                            CUnitValue cuv;
                        ...
                        // into scope
                        if(!fNeedsLayout && pPF->HasPadBord(FALSE))
                        {
                        ...
                        if ((!pMe->_li._fHasBulletOrNum && !fFirstLineInLayout)
                            || pFF->_fExplicitTopMargin)
                        {
                                ...
                                pNode->Parent()->GetParaFormat()->HasPre(FALSE)))
                            {
                                ...
                                lTemp = pFF->_cuvSpaceBefore.YGetPixelValue(
                                    _pci, _pci->_sizeParent.cx, lFontHeight);
...
//
//-----------------------------------------------------------------------------
BOOL CRecalcLinePtr::CollectSpaceInfoFromEndNode(CTreeNode* pNode, BOOL fFirstLineInLayout, BOOL fPadBordForEmptyBlock)
{
    Assert(pNode);
    ...
    if(!fFirstLineInLayout &&
        (pElement->_fExplicitEndTag || pFF->_fExplicitBottomMargin))
    {
    ...
    // going out of scope
    if(!pNode->NeedsLayout() && pNode->GetParaFormat()->HasPadBord(FALSE))
    {
