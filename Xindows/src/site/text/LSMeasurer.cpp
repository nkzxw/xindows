...
//
//+----------------------------------------------------------------------------
void CLSMeasurer::Init(const CDisplay* pdp, CCalcInfo* pci, BOOL fStartUpLSDLL)
{
...
//
//+----------------------------------------------------------------------------
void CLSMeasurer::Reinit(const CDisplay* pdp, CCalcInfo* pci)
{
    CTreePos *ptpFirst, *ptpLast;
...
//
//+----------------------------------------------------------------------------
void CLSMeasurer::SetCalcInfo(CCalcInfo* pci)
{
    if(!pci)
    {
...
//
//+----------------------------------------------------------------------------
CCalcInfo* CLSMeasurer::CalcInfoFromDocInfo(const CDisplay* pdp, CDocInfo* pdci)
{
    _pFlowLayout = pdp->GetFlowLayout();
    if(!pdci)
    {
...
//
//-----------------------------------------------------------------------------
void CLSMeasurer::SetCp(LONG cp, CTreePos* ptp)
{
    _cp = cp;
    if(ptp == NULL)
    {
...
//
//-----------------------------------------------------------------------------
void CLSMeasurer::SetPtp(CTreePos* ptp, LONG cp)
{
    Assert(ptp);
    _ptpCurrent = ptp;
...
//
//-----------------------------------------------------------------------------
void CLSMeasurer::Advance(long cch, CTreePos* ptp /* = NULL */)
{
    _cp += cch;
    if(ptp)
    {
...
//
//-----------------------------------------------------------------------------
void CLSMeasurer::InitLineSpace(const CLSMeasurer* pMe, CLinePtr& rpOld)
{
    // Zero it all out.
    NewLine(FALSE);
...
//
//-----------------------------------------------------------------------------
void CLSMeasurer::NewLine(BOOL fFirstInPara)
{
...
//
//-----------------------------------------------------------------------------
void CLSMeasurer::NewLine(const CLine&li)
{
    _li           = li;
    _li._cch      = 0;
    _li._cchWhite = 0;
...
//
//-----------------------------------------------------------------------------
LONG CLSMeasurer::MeasureRangeOnLine(
...
//
//-----------------------------------------------------------------------------
HRESULT CLSMeasurer::LSDoCreateLine(
                            LONG cp,                  // IN
                            CTreePos* ptp,            // IN
                            ...
                            LSLINFO* plslinfo)        // OUT
{
    ...
    hr = _pLS->Setup(xWidthMaxIn, cp, ptp, pMarginInfo, pli, fMinMaxPass);
    if(hr != S_OK)
    {
        goto Cleanup;
    }
                       ...
                       LONG* pxMinLineWidth)
{
    ...
    hr = LSDoCreateLine(GetCp(), GetPtp(), pMarginInfo, xWidthMaxIn, NULL,
    ...
    // the flags. We will then pass these flags down to ComputeWhiteInfo.
    {
        ...
        // concert for compat.
        if(fLineHasBR && !_pLS->_li._fJustified)
        {
        ...
        if((dwlf&FLAG_HAS_RELATIVE) && lslinfo.endr==endrNormal)
        {
    ...
    if(pxMinLineWidth)
    {
        ...
        *pxMinLineWidth += _pLS->_li._xLeft + _pLS->_li._xRight;
...
//
//-----------------------------------------------------------------------------
LONG CLSMeasurer::InitForMeasure(UINT uiFlags)
{
                          ...
                          LONG* pxMinLineWidth)
{
    ...
    _pLS->SetMeasurer(this,
        uiFlags&MEASURE_BREAKWORDS?CLineServices::LSMODE_HITTEST:CLineServices::LSMODE_MEASURER,
    ...
    // let's auto clear.
    if(pli->_xWidth>xWidthMax && (uiFlags&MEASURE_AUTOCLEAR))
    {
    ...
    if(!pli->_fHidden
        ...
        || ((pli->_fHasBreak||pli->_fHasBulletOrNum)
        && pli->_cch <= _pLS->CchSkipAtBOL())))
    {
...
CLine* CLSMeasurer::AccountForRelativeLines(
    CLine& li,                     // IN
...
BOOL CLSMeasurer::AdvanceToNextAlignedObject()
{
    BOOL        fFound = FALSE;
    CTreePos*   ptp;
    CTreeNode*  pNode;
                                      ...
                                      UINT uiFlags)                 // MEASURE_ flags
{
    ...
    *ppliOut = AccountForRelativeLines(
        *pliIn,
...
BOOL IsListItem(CElement* pElement)
{
...
void CLSMeasurer::MeasureListIndent()
{
    ...
    if(_li._fHasBulletOrNum)
    {
        ...
        if(pPF->GetImgCookie(fInner) && MeasureImage(pPF->GetImgCookie(fInner), &sizeImg))
        {
...
CCcs* CLSMeasurer::GetCcsSymbol(TCHAR chSymbol, const CCharFormat* pcf, CCharFormat* pcfRet)
{
    CCcs*           pccs = NULL;
    CCharFormat     cf;
    ...
    *pcfUsed = s_cfBullet;
    ...
    pccs->GetBaseCcs()->_bConvertMode = CVM_SYMBOL;

#ifdef _DEBUG
    if(!pccs)
    {
        Trace0("CRchMeasurer::GetCcsBullet(): no CCcs\n");
    }
#endif
...
//
//-----------------------------------------------------------------------------
CCcs* CLSMeasurer::GetCcsNumber(const CCharFormat* pCF, CCharFormat* pCFRet)
{
    CCharFormat cf = *pCF;
...
void CLSMeasurer::MeasureSymbol(TCHAR chSymbol, LONG* pxWidth)
{
    ...
    if(pccs)
    {
        ...
        {
            Trace0("CLSMeasurer::MeasureSymbol(): Error filling CCcs\n");
        }
...
BOOL CLSMeasurer::MeasureImage(long lImgCookie, SIZE* psizeImg)
{
    CDocument* pDoc = _pFlowLayout->Doc();
...
//
//-----------------------------------------------------------------------------
BOOL CLSMeasurer::TestForClear(const CMarginInfo* pMarginInfo, LONG cp, BOOL fShouldMarginsExist, const CFancyFormat* pFF)
{
