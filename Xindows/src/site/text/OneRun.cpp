...
//
//-----------------------------------------------------------------------------
CBidiLine::CBidiLine(const CTreeInfo& treeinfo, LONG cp, BOOL fRTLPara, const CLine* pli)
...
//
//-----------------------------------------------------------------------------
CBidiLine::CBidiLine(BOOL fRTLPara, LONG cchText, const WCHAR* pchText)
{
#ifdef NO_IMPLICIT_BIDI
    DIR_RUN dr;
...
//
//-----------------------------------------------------------------------------
DIRCLS CBidiLine::GetInitialDirClass(BOOL fRTLLine)
{
    if(!fRTLLine)
    {
    ...
    case CP_UCS_4_BIGENDIAN:
        ...
        // the DIRCLS to ARA.
        if(IsInArabicBlock(PRIMARYLANGID(_afxGlobalData._lcidUserDefault)))
        {
            return ARA;
        }
    }
...
//
//-----------------------------------------------------------------------------
LONG CBidiLine::GetRunCchRemaining(LONG cp, LONG cchMax)
{
#ifdef NO_IMPLICIT_BIDI
    return cchMax;
#else
...
//
//-----------------------------------------------------------------------------
const DIR_RUN& CBidiLine::GetDirRun(LONG cp)
{
#ifdef NO_IMPLICIT_BIDI
    return _aryDirRun[0];
#else
...
//
//-----------------------------------------------------------------------------
LONG CBidiLine::FindRun(LONG cp)
{
...
//
//-----------------------------------------------------------------------------
void CBidiLine::LogicalToVisual(BOOL fRTLBuffer, LONG cchText, const WCHAR* pchLogical, WCHAR* pchVisual)
{
    LONG iLev = 0;
    LONG iRun;
...
//
//-----------------------------------------------------------------------------
void CBidiLine::EvaluateLayoutToCp(LONG cp, LONG cpLine)
{
    ...
    while(_cp <= cp)
    {
        ...
        cchFetch = (cpLine==-1) ? DEFAULT_CHARS_TO_EVALUATE :
...
//
//-----------------------------------------------------------------------------
void CBidiLine::AdjustTrailingWhitespace(LONG cpEOL)
{
    LONG iRun;
    BOOL fWhite;
    LONG cchValid;
...
//
//-----------------------------------------------------------------------------
LONG CBidiLine::GetTextForLayout(
                                 WCHAR* pch,
                                 LONG cch,
                                 LONG cp,
                                 CTreePos** pptp,
        ...
        case CTreePos::NodeBeg:
            {
                ...
                if(pElement->HasFlag(TAGDESC_OWNLINE) ||
                    pElement->Tag()==ETAG_BR || pElement->IsBlockElement())
                {
                    ...
                    if(!pElement->HasLayout())
                    {
                        ...
                        if(pCF->_fBidiEmbed)
                        {
                            ...
                            *pch = ((!pCF->_fBidiOverride) ?
                                (!pCF->_fRTL?WCH_LRE:WCH_RLE) : (!pCF->_fRTL?WCH_LRO:WCH_RLO));
                        }
                        pch++;
                    }
...
//
//-----------------------------------------------------------------------------
const DIRCLS CBidiLine::s_adcEmbedding[5] =
{
...
//
//-----------------------------------------------------------------------------
const DIRCLS CBidiLine::s_adcNumeric[3] =
{
    ENL,    // LTR ENM
    ENR,    // RTL ENM
    ANM,    // ARA ENM
...
//
//-----------------------------------------------------------------------------
const DIRCLS CBidiLine::s_adcNeutral[2][6][6] =
{
...
//
//-----------------------------------------------------------------------------
const BYTE CBidiLine::s_abLevelOffset[2][6] =
{
...
BYTE CchContextFromScript(WORD eScript)
{
...
//
//-----------------------------------------------------------------------------
void CComplexRun::ComputeAnalysis(
                                  ...
                                  DWORD uLangDigits)
{
    CStackDataAry<SCRIPT_ITEM, 64> aryItem;
    CString strText;
    ...
    {
        if(strText.Set(pch, cch)==S_OK && strText.Append(&chNext, 1)==S_OK)
        {
            pch = strText;
            cch++;
        }
    ...
    if(FAILED(hr))
    {
        if(hr == USP10_NOT_FOUND)
        {
            ...
            if(g_bUSPJitState == JIT_OK)
            {
                ...
                GWPostMethodCall(pFlowLayout->Doc(),
    ...
    if(_Analysis.eScript != SCRIPT_UNDEFINED)
    {
        ...
        if(IsComplexScript(_Analysis.eScript) && pCFBase->_bCharSet!=bScriptCharSet)
        {
            CCharFormat* pCF;
...
//
//-----------------------------------------------------------------------------
BOOL CComplexRun::IsNumericSeparatorRun(COneRun* por, COneRun* porTail)
{
    ...
    else
    {
        ...
        while((ch=txtptr.NextChar()) == WCH_NODE)
        {
            ...
            if(pElement->HasFlag(TAGDESC_OWNLINE) ||
                pElement->Tag()==ETAG_BR || pElement->HasLayout())
            {
                return FALSE;
            }
        }
...
//
//-----------------------------------------------------------------------------
void CComplexRun::ThaiTypeBrkcls(CMarkup* pMarkup, LONG cp, BRKCLS* pbrkclsAfter, BRKCLS* pbrkclsBefore)
{
    long cchBefore = 0;
    long cchAfter = 0;
    ...
    if(cp < (LONG)_ThaiWord.cp || cp>=(LONG)(_ThaiWord.cp+_ThaiWord.cch))
    {
        ...
        for(nItem=aryItems.Size()-1; cchBefore < aryItems[nItem].iCharPos; nItem--) ;
...
//
//-----------------------------------------------------------------------------
void COneRun::Selected(CFlowLayout* pFlowLayout, BOOL fSelected)
{
...
//
//-----------------------------------------------------------------------------
void COneRun::ImeHighlight(CFlowLayout* pFlowLayout, DWORD dwImeHighlight)
{
...
CCharFormat* COneRun::GetOtherCF()
{
    if(!_fMustDeletePcf)
    {
...
//
//-----------------------------------------------------------------------------
void COneRun::SetCurrentBgColor(CFlowLayout* pFlowLayout)
{
    CTreeNode* pNode = Branch();
    CElement* pElementFL = _fCharsForNestedRunOwner ?
        Branch()->Element(): pFlowLayout->ElementOwner();
...
//
//-----------------------------------------------------------------------------
COneRun* COneRun::Clone(COneRun* porClone)
{
    COneRun* porRet = this;
    ...
    memset(&_cstrRunChars, 0, sizeof(CString));
...
//
//-----------------------------------------------------------------------------
void COneRun::FillSynthData(CLineServices::SYNTHTYPE synthtype)
{
...
//
//-----------------------------------------------------------------------------
COneRun::charWidthClass COneRun::GetCharWidthClass()
{
