...
#include "stdafx.h"
#include "DispScroller.h"

void CDispScroller::GetVScrollbarRect(CRect* prcVScrollbar, const CDispInfo& di) const
{
    if(!IsRightToLeft())
    {
        prcVScrollbar->right = _rcContainer.Width() - di._prcBorderWidths->right;
        prcVScrollbar->left =
            max(di._prcBorderWidths->left, prcVScrollbar->right-_sizeScrollbars.cx);
    }
    else
    {
        prcVScrollbar->left = di._prcBorderWidths->left;
        prcVScrollbar->right =
            min(_rcContainer.Width()-di._prcBorderWidths->right, prcVScrollbar->left+_sizeScrollbars.cx);
    }
    prcVScrollbar->top = di._prcBorderWidths->top;
    prcVScrollbar->bottom = _rcContainer.Height() - di._prcBorderWidths->bottom;
    if(_fHasHScrollbar)
	{
...
void CDispScroller::GetHScrollbarRect(CRect* prcHScrollbar, const CDispInfo& di) const
{
    prcHScrollbar->bottom = _rcContainer.Height() - di._prcBorderWidths->bottom;
    prcHScrollbar->top =
        max(di._prcBorderWidths->top, prcHScrollbar->bottom-_sizeScrollbars.cy);
    prcHScrollbar->left = di._prcBorderWidths->left;
    prcHScrollbar->right = _rcContainer.Width() - di._prcBorderWidths->right;
...
//
//----------------------------------------------------------------------------
void CDispScroller::SetVerticalScrollbarWidth(LONG width, BOOL fForce)
{
    Assert(width >= 0);
...
//
//----------------------------------------------------------------------------
void CDispScroller::SetHorizontalScrollbarHeight(LONG height, BOOL fForce)
{
    Assert(height >= 0);
...
//
//----------------------------------------------------------------------------
BOOL CDispScroller::VerticalScrollbarIsActive() const
{
    if(!_fHasVScrollbar)
	{
		return FALSE;
	...
	return
		...
		-((_fHasHScrollbar)?_sizeScrollbars.cy:0) < _sizeContent.cy;
}
...
//
//----------------------------------------------------------------------------
BOOL CDispScroller::HorizontalScrollbarIsActive() const
{
    if(!_fHasHScrollbar)
	{
		return FALSE;
    ...
    return
        ...
        -((_fHasVScrollbar)?_sizeScrollbars.cx:0) < _sizeContent.cx;
}
...
//
//----------------------------------------------------------------------------
BOOL CDispScroller::CalcScrollbars()
{
    ...
    CSize sizeInsideBorder(
        _rcContainer.Width()-di._prcBorderWidths->left-di._prcBorderWidths->right,
        _rcContainer.Height()-di._prcBorderWidths->top-di._prcBorderWidths->bottom);

    // determine whether vertical scroll bar is needed
    _fHasVScrollbar = _sizeScrollbars.cx>0 && (_fForceVScrollbar||_sizeContent.cy>sizeInsideBorder.cy);
    ...
    if(_fHasVScrollbar)
    {
        ...
        _fHasHScrollbar = _sizeScrollbars.cy>0 && (_fForceHScrollbar||_sizeContent.cx>sizeInsideBorder.cx);
    ...
    else
    {
        ...
        if(_fHasHScrollbar)
        {
            ...
            _fHasVScrollbar = _sizeScrollbars.cx>0 && _sizeContent.cy>sizeInsideBorder.cy;
...
//
//----------------------------------------------------------------------------
void CDispScroller::InvalidateScrollbars()
{
    CDispInfo di(GetExtras());
    CRegion rgnInvalid;
...
void CDispScroller::InvalidateScrollbars(CRegion* prgnInvalid, CDispInfo* pdi)
{
    if(_fInvalidVScrollbar)
    {
        ...
        if(_fInvalidHScrollbar)
        {
            ...
            CRect rcInsideBorder(
                _rcContainer.TopLeft()+pdi->_prcBorderWidths->TopLeft().AsSize(),
                _rcContainer.BottomRight()-pdi->_prcBorderWidths->BottomRight().AsSize());
            CRect rcContent(
...
//
//----------------------------------------------------------------------------
void CDispScroller::SetSize(const SIZE& size, BOOL fInvalidateAll)
{
...
//
//----------------------------------------------------------------------------
BOOL CDispScroller::ScrollRectIntoView(
        const CRect& rc,
        ...
        BOOL fScrollBits)
{
    ...
    else
	{
		...
		contentOffset.SetSize(pt.x-di._prcBorderWidths->left, pt.y+di._prcBorderWidths->top);
    }
    ...
    if(!fRightToLeft)
    {
		rcView.SetRect(contentOffset.AsPoint()-_sizeScrollOffset,
			_rcContainer.BottomRight()-di._prcBorderWidths->BottomRight().AsSize()-_sizeScrollOffset);
	}
	else
	{
		rcView.SetRect(
            _rcContainer.TopLeft()+di._prcBorderWidths->TopLeft().AsSize()-_sizeScrollOffset,
			_rcContainer.BottomRight()-di._prcBorderWidths->BottomRight().AsSize()-_sizeScrollOffset);
	}
    ...
    CSize scrollOffset;
    BOOL fScrolled = rcView.CalcScrollDelta(rcParent, &scrollOffset, spVert, spHorz);
    if(fScrolled)
    {
...
//
//----------------------------------------------------------------------------
BOOL CDispScroller::PreDraw(CDispDrawContext* pContext)
{
    ...
    if(pContext->IntersectsRedrawRegion(_rcVisBounds) && super::PreDraw(pContext))
    {
        return TRUE;
    }
...
//
//----------------------------------------------------------------------------
void CDispScroller::DrawSelf(CDispDrawContext* pContext, CDispNode* pChild)
{
...
//
//----------------------------------------------------------------------------
void CDispScroller::DrawBorderAndScrollbars(CDispDrawContext* pContext, CRect* prcContent)
{
...
//
//----------------------------------------------------------------------------
void CDispScroller::DrawScrollbars(CDispDrawContext* pContext, DWORD dwFlags)
{
    CDispInfo di(GetExtras());
    CRect rcVScrollbar(CRect::CRECT_EMPTY);
    CRect rcHScrollbar(CRect::CRECT_EMPTY);
...
//
//----------------------------------------------------------------------------
BOOL CDispScroller::HitTestPoint(CDispHitContext* pContext) const
{
    ...
    if(IsVisible() && (_fHasVScrollbar || _fHasHScrollbar))
    {
        ...
        pContext->_ptHitTest -= _rcContainer.TopLeft().AsSize();
        CRect rcVScrollbar(CRect::CRECT_EMPTY);
        CRect rcHScrollbar(CRect::CRECT_EMPTY);
...
CDispScroller* CDispScroller::HitScrollInset(CPoint* pptHit, DWORD* pdwScrollDir)
{
    ...
    *pdwScrollDir = 0;
    ...
    if(sizeInsideBorder.cx > 2*g_sizeDragScrollInset.cx)
    {
		if((pptHit->x<=g_sizeDragScrollInset.cx)
			&& (pptHit->x>=0) && (_sizeScrollOffset.cx<0))
		{
    ...
    if(sizeInsideBorder.cy > 2*g_sizeDragScrollInset.cy)
    {
        if((pptHit->y<=g_sizeDragScrollInset.cy)
            && (pptHit->y>=0) && (_sizeScrollOffset.cy<0))
        {
...
//
//----------------------------------------------------------------------------
void CDispScroller::GetClientRect(RECT* prc, CLIENTRECT type) const
{
    CDispInfo di(GetExtras());
    BOOL fRightToLeft = IsRightToLeft();
    ...
    case CLIENTRECT_VISIBLECONTENT:
        {
            ...
            ((CRect*)prc)->SetRect(-_sizeScrollOffset.AsPoint(), sizeContained);
            if(fRightToLeft)
            {
...
//
//----------------------------------------------------------------------------
BOOL CDispScroller::SetScrollOffset(const SIZE& offset, BOOL fScrollBits)
{
    CDispInfo di(GetExtras());
    ...
    if(!fRightToLeft)
    {
        ...
        sizeDiff.cx =
            (sizeContainer.cx>=_sizeContent.cx ? 0
                : max(-peggedOffset.cx, sizeContainer.cx-_sizeContent.cx))-_sizeScrollOffset.cx;
    }
    else
    {
        ...
        sizeDiff.cx =
            (sizeContainer.cx>=_sizeContent.cx ? 0
                : min(-peggedOffset.cx, _sizeContent.cx-sizeContainer.cx))-_sizeScrollOffset.cx;
    }
    ...
    sizeDiff.cy =
        (sizeContainer.cy>=_sizeContent.cy ? 0
            : max(-peggedOffset.cy, sizeContainer.cy-_sizeContent.cy))-_sizeScrollOffset.cy;
    ...
    else
    {
        ...
        for(;;)
        {
            for(CDispNode* pSibling=pNode->_pNextSiblingNode;
                 pSibling!=NULL; pSibling=pSibling->_pNextSiblingNode)
            {
...
BOOL CDispScroller::SetScrollOffsetSmoothly(const SIZE& offset, BOOL fScrollBits, LONG lScrollTime)
{
    CDispNode* pNode;
    CDispRoot* pRoot;

    // determine if smooth scrolling is enabled
    if(!fScrollBits || lScrollTime<=0 || !AllSet(CDispFlags::s_visibleBranchAndInView))
	{
		goto NoSmoothScroll;
	}
    pNode = GetRootNode();
...

    {
        ...
        LONG cScrollsDone = 0;
        CSize perScroll(_afxGlobalData._Zero.size);
...
//
//----------------------------------------------------------------------------
BOOL CDispScroller::ComputeVisibleBounds()
{
...
//
//----------------------------------------------------------------------------
void CDispScroller::RecalcChildren(BOOL fForceRecalc, BOOL fSuppressInval, CDispDrawContext* pContext)
{
    ...
    CDispInfo di(GetExtras());
    CRect rcVSB(_afxGlobalData._Zero.rc);
    CRect rcHSB(_afxGlobalData._Zero.rc);
    ...
    if(!fSuppressInval && IsVisible())
    {
        ...
        {
            CRect rc;
            CalcDispInfo(_afxGlobalData._Zero.rc, &di);
			if(_fInvalidVScrollbar || (_fHasVScrollbar && _sizeContent.cy!=sizeContentOld.cy))
			{
            ...
            if(_fInvalidHScrollbar || (_fHasHScrollbar && _sizeContent.cx!=sizeContentOld.cx))
            {
                ...
                // scroll bars.
                if(rcVSB != _afxGlobalData._Zero.rc)
                {
                    rc.SetRect(
                        rcVSB.left,
                        rcHSB.top,
...
//
//----------------------------------------------------------------------------
void CDispScroller::CalcDispInfo(const CRect& rcClip, CDispInfo* pdi) const
{
...
//
//----------------------------------------------------------------------------
void CDispScroller::GetNodeTransform(CSize* pOffset, COORDINATE_SYSTEM source, COORDINATE_SYSTEM destination) const
{
...
//
//----------------------------------------------------------------------------
void CDispScroller::GetNodeTransform(CDispContext* pContext, COORDINATE_SYSTEM source, COORDINATE_SYSTEM destination) const
{
    Assert(destination < source);
    BOOL fRightToLeft = IsRightToLeft();
