...
//
//----------------------------------------------------------------------------
void CDispContainer::SetSize(const SIZE& size, const RECT& rcBorderWidths,
                             BOOL fInvalidateAll)
{
    // caller should call SetRootSize on CDispRoot
    Assert(GetNodeType() != DISPNODETYPE_ROOT);

    CSize sizeDelta(size.cx-_rcContainer.Width(), size.cy-_rcContainer.Height());
    if(sizeDelta.IsZero())
	{
		return;
	}
...
//
//----------------------------------------------------------------------------
void CDispContainer::SetPosition(const POINT& ptTopLeft)
{
...
//
//----------------------------------------------------------------------------
void CDispContainer::SetPositionTopRight(const POINT& ptTopRight)
{
    CSize offset(ptTopRight.x-_rcContainer.right, ptTopRight.y-_rcContainer.top);
...
//
//----------------------------------------------------------------------------
void CDispContainer::GetClientRect(RECT* prc, CLIENTRECT type) const
{
    switch(type)
    {
...
//
//----------------------------------------------------------------------------
BOOL CDispContainer::PreDraw(CDispDrawContext* pContext)
{
    ...
    pContext->_offset += (di._borderOffset + di._contentOffset + di._scrollOffset);

    // continue predraw traversal of children, top layers to bottom
    int lastLayer = (int)DISPNODELAYER_POSITIVEZ + 1;
    for(CDispNode* pChild=_pLastChildNode; pChild!=NULL;
...
//              
//----------------------------------------------------------------------------
void CDispContainer::DrawClientLayer(
...
//
//----------------------------------------------------------------------------
void CDispContainer::DrawSelf(CDispDrawContext* pContext, CDispNode* pChild)
{
    // Interesting nodes are visible, in-view, opaque
...
//
//----------------------------------------------------------------------------
void CDispContainer::DrawBorderAndScrollbars(CDispDrawContext* pContext, CRect* prcContent)
{
    // draw optional border
    CDispExtras* pExtras = GetExtras();
    if(HasBorder(pExtras))
...
//              
//----------------------------------------------------------------------------
void CDispContainer::DrawBorder(CDispDrawContext* pContext, CDispInfo* pDI)
{
    Assert(HasBorder());
...
//              
//----------------------------------------------------------------------------
void CDispContainer::DrawBackground(CDispDrawContext* pContext, CDispInfo* pDI)
{
    if(IsVisible())
    {
        CDispContext saveContext(*pContext);
...
//              
//----------------------------------------------------------------------------
void CDispContainer::DrawChildren(
        DISPNODELAYER layer,
        const CSize& offset,
        const CRect& rcClip,
        CDispDrawContext* pContext,
        CDispNode** ppChildNode)
{
    ...
    for(; pChild!=NULL&&pChild->GetLayerType()==layer;
...
//
//----------------------------------------------------------------------------
BOOL CDispContainer::HitTestPoint(CDispHitContext* pContext) const
{
...

    {
        ...
        pContext->_ptHitTest -= di._borderOffset + di._contentOffset + di._scrollOffset;
        ...
        for(CDispNode* pChild=_pLastChildNode; pChild!=NULL; pChild=pChild->_pPreviousSiblingNode)
        {
            ...
            // Edit team is to provide a better UI-level way of dealing with this problem for post IE5.
            // BUGBUG revert sig. of FuzzyRectIsHit to not take the extra param
            if(pContext->FuzzyRectIsHit(rcChild, pChild->IsFatHitTest()) && pChild->HitTestPoint(pContext))
            {
    ...
    if(IsVisible())
    {
        pContext->_ptHitTest = ptSave - di._borderOffset -
            di._contentOffset - di._scrollOffset;
        if(pContext->RectIsHit(di._rcBackgroundClip))
        {
			if(_pDispClient->HitTestContent(&pContext->_ptHitTest,
                (CDispContainer*)this, pContext->_pClientData))
			{
                // NOTE: don't bother to restore _ptHitTest for speed
                return TRUE;
            }
        ...
        else if(HasBorder(pExtras))
        {
            ...
            {
				if(_pDispClient->HitTestBorder(&pContext->_ptHitTest,
                    (CDispContainer*)this, pContext->_pClientData))
				{
                    // NOTE: don't bother to restore _ptHitTest for speed
                    return TRUE;
...
CDispScroller* CDispContainer::HitScrollInset(CPoint* pptHit, DWORD* pdwScrollDir)
{
...
//
//----------------------------------------------------------------------------
void CDispContainer::CalcDispInfo(const CRect& rcClip, CDispInfo* pdi) const
{
...
//
//----------------------------------------------------------------------------
void CDispContainer::GetNodeTransform(CSize* pOffset, COORDINATE_SYSTEM source,
    ...
    default:
        ...
        if(_pParentNode != NULL)
        {
            ...
            _pParentNode->GetNodeTransform(
                &globalOffset, GetContentCoordinateSystem(), COORDSYS_GLOBAL);
...
//
//----------------------------------------------------------------------------
void CDispContainer::GetNodeTransform(CDispContext* pContext, COORDINATE_SYSTEM source,
                                      COORDINATE_SYSTEM destination) const
{
    Assert(destination < source);
    BOOL fRightToLeft = IsRightToLeft();
    ...
    case COORDSYS_CONTENT:
        ...
        offset = _afxGlobalData._Zero.size;
    ...
    default:
        ...
        if(source == COORDSYS_PARENT)
        {
            ...
            {
                _pParentNode->GetNodeTransform(
                    pContext, GetContentCoordinateSystem(), COORDSYS_GLOBAL);
            }
            else
			{
        ...
        else
        {
            if(_pParentNode != NULL)
            {
                ...
                _pParentNode->GetNodeTransform(
                    &globalContext, GetContentCoordinateSystem(), COORDSYS_GLOBAL);
...
//
//----------------------------------------------------------------------------
void CDispContainer::PushContext(
...
//
//----------------------------------------------------------------------------
BOOL CDispContainer::ComputeVisibleBounds()
{
...
//
//----------------------------------------------------------------------------
BOOL CDispContainer::SubtractOpaqueChildren(CRegion* prgn, CDispContext* pContext)
{
...
//
//----------------------------------------------------------------------------
BOOL CDispContainer::CalculateInView(CDispContext* pContext, BOOL fPositionChanged,
                                     BOOL fNoRedraw)
{
    // apply user clip
...
//
//----------------------------------------------------------------------------
void CDispContainer::RecalcChildren(BOOL fForceRecalc, BOOL fSuppressInval,
                                    CDispDrawContext* pContext)
{
    CDispContext saveContext(*pContext);
