...
//
//----------------------------------------------------------------------------
void CDispItemPlus::SetSize(const SIZE& size, BOOL fInvalidateAll)
{
...
//
//----------------------------------------------------------------------------
BOOL CDispItemPlus::ScrollRectIntoView(
        const CRect& rc,
...
//
//----------------------------------------------------------------------------
BOOL CDispItemPlus::CalculateInView(
        ...
        BOOL fNoRedraw)
{
    ...
    if(AllSet(CDispFlags::s_inViewChangeAndVisibleBranch) && (fInView||fWasInView))
    {
        if(fPositionChanged)
        {
...
//
//----------------------------------------------------------------------------
void CDispItemPlus::DrawSelf(CDispDrawContext* pContext, CDispNode* pChild)
{
         ...
         (dwClientLayers&(CLIENTLAYERS_BEFOREBACKGROUND|CLIENTLAYERS_AFTERBACKGROUND)))
    {
        ...
        if(!rcClip.IsEmpty())
        {
            ...
            {
                DrawClientLayer(pContext, di, rcClip, CLIENTLAYERS_BEFOREBACKGROUND);
            }
            ...
            {
                DrawClientLayer(pContext, di, rcClip, CLIENTLAYERS_AFTERBACKGROUND);
            }
        }
    }
    ...
    if(!rcClip.IsEmpty())
    {
        ...
        else
        {
            ...
            {
                DrawClientLayer(pContext, di, rcClip, CLIENTLAYERS_BEFORECONTENT);
            }
            ...
            {
                DrawClientLayer(pContext, di, rcClip, CLIENTLAYERS_AFTERCONTENT);
            }
            if(dwClientLayers & CLIENTLAYERS_AFTERFOREGROUND)
            {
                DrawClientLayer(pContext, di, rcClip, CLIENTLAYERS_AFTERFOREGROUND);
            }
        }
    }
...
//              
//----------------------------------------------------------------------------
void CDispItemPlus::DrawBorder(CDispDrawContext* pContext, CDispInfo* pDI)
{
...
//              
//----------------------------------------------------------------------------
void CDispItemPlus::DrawBackground(CDispDrawContext* pContext, CDispInfo* pDI)
{
    CDispContext saveContext(*pContext);
...
//              
//----------------------------------------------------------------------------
void CDispItemPlus::DrawContent(CDispDrawContext* pContext, CDispInfo* pDI)
{
    CDispContext saveContext(*pContext);
...
//              
//----------------------------------------------------------------------------
void CDispItemPlus::DrawClientLayer(
            ...
            DWORD dwClientLayer)
{
    ...
    if(IsRightToLeft())
    {
        ...
        long offset = _rcVisBounds.Width() - di._prcBorderWidths->left - di._prcBorderWidths->right;
...
//
//----------------------------------------------------------------------------
BOOL CDispItemPlus::HitTestPoint(CDispHitContext* pContext) const
{
	...
	{
		if(_pDispClient->HitTestBorder(&pContext->_ptHitTest, (CDispNode*)this, pContext->_pClientData))
        {
    ...
    {
        pContext->_ptHitTest -= *di._pInsetOffset + di._contentOffset + di._scrollOffset;
        ...
        if(pContext->RectIsHit(di._rcBackgroundClip))
        {
			...
			if(_pDispClient->HitTestContent(
				&(pContext->_ptHitTest), (CDispLeafNode*)this, pContext->_pClientData))
            {
                // NOTE: don't bother to restore _ptHitTest for speed
                return TRUE;
            }
...
//
//----------------------------------------------------------------------------
void CDispItemPlus::CalcDispInfo( const CRect& rcClip, CDispInfo* pdi) const
{
...
//
//----------------------------------------------------------------------------
void CDispItemPlus::GetNodeTransform(
        CSize* pOffset,
    ...
    case COORDSYS_CONTENT:
        // COORDSYS_CONTENT --> COORDSYS_CONTAINER
        {
            ...
            if(!fRightToLeft)
            {
                ...
                *pOffset = di._prcBorderWidths->TopLeft().AsSize() + *di._pInsetOffset;
            }
            else
            {
        ...
        COORDINATE_SYSTEM destination) const
{
    ...
    case COORDSYS_CONTENT:
        // COORDSYS_CONTENT --> COORDSYS_CONTAINER
        {
            ...
            CSize sizeContained =
                _rcVisBounds.Size() - topLeftBorder - di._prcBorderWidths->BottomRight().AsSize();
...
//
//----------------------------------------------------------------------------
void CDispItemPlus::GetClientRect(RECT* prc, CLIENTRECT type) const
{
    switch(type)
    {
...
//
//----------------------------------------------------------------------------
LONG CDispItemPlus::GetZOrder() const
{
...
//
//----------------------------------------------------------------------------
void CDispItemPlus::NotifyInViewChange(
        CDispContext* pContext,
        ...
        BOOL fNoRedraw)
{
    ...
    if(!IsRightToLeft())
    {
        CRect rcTemp(_rcVisBounds.TopLeft()+di._contentOffset+di._scrollOffset+*di._pInsetOffset+pContext->_offset,
