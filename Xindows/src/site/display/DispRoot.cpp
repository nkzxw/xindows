...
//
//----------------------------------------------------------------------------
CDispRoot::CDispRoot(IDispObserver* pObserver, CDispClient* pDispClient) : CDispContainer(pDispClient)
{
...
//
//----------------------------------------------------------------------------
void CDispRoot::DrawEntire(CDispDrawContext* pContext)
{
...
void CDispRoot::DrawBand(CDispDrawContext* pContext)
{
    // pop first node and redraw region
    CDispNode* pDrawNode = (CDispNode*)pContext->PopFirstRedrawRegion();
...
CDispItemPlus* CDispRoot::CreateDispItemPlus(
        ...
        BOOL fRightToLeft)
{
    size_t extrasSize = CDispExtras::GetExtrasSize(fHasExtraCookie, fHasUserClip, fHasInset, borderType);
    CDispItemPlus* pItem = new (extrasSize) CDispItemPlus(pDispClient);
    if(pItem != NULL)
    {
...
CDispContainer* CDispRoot::CreateDispContainer(
        CDispClient* pDispClient,
        BOOL fHasExtraCookie,
        BOOL fHasUserClip,
        BOOL fHasInset,
...
CDispContainer* CDispRoot::CreateDispContainer(const CDispItemPlus* pItemPlus)
{
...
CDispScroller* CDispRoot::CreateDispScroller(
        CDispClient* pDispClient,
        BOOL fHasExtraCookie,
        BOOL fHasUserClip,
        BOOL fHasInset,
...
//
//----------------------------------------------------------------------------
void CDispRoot::ReleaseOffscreenBuffer()
{
...
//
//----------------------------------------------------------------------------
BOOL CDispRoot::ScrollRectIntoView(
        const CRect& rc,
        ...
        long cFuzzyHitTest)
{
    ...
    {
        CPoint ptHit(*pptHit);
        if(coordinateSystem==COORDSYS_GLOBAL || coordinateSystem==COORDSYS_PARENT)
        {
        ...
        // of root.
        BOOL fHit =
            _pFirstChildNode->HitTest(&ptHit, COORDSYS_PARENT, pClientData, fHitContent, cFuzzyHitTest);
        if(fHit)
		{
			*pptHit = ptHit;
		}
...
void CDispRoot::CloseDisplayTree()
{
    CheckReenter();
    Assert(_cOpen > 0);
    if(_cOpen == 1)
...
//              
//----------------------------------------------------------------------------
void CDispRoot::InvalidateRoot(const CRect& rcInval, BOOL fSynchronousRedraw, BOOL fInvalChildWindows)
{
...
//              
//----------------------------------------------------------------------------
void CDispRoot::InvalidateRoot(const CRegion& rgn, BOOL fSynchronousRedraw, BOOL fInvalChildWindows)
{
    AssertSz(!_fDrawLock, "Illegal call to CDispRoot inside Draw()");
...
//              
//----------------------------------------------------------------------------
void CDispRoot::SetOffscreenBufferInfo(HPALETTE hpal, short bufferDepth, BOOL fDirectDraw, BOOL fTexture, BOOL fWantOffscreen, BOOL fAllowOffscreen)
{
...
//              
//----------------------------------------------------------------------------
BOOL CDispRoot::SetupOffscreenBuffer(CDispDrawContext* pContext)
{
    Assert(pContext->GetDispSurface() == _pRenderSurface);
    DEBUG_ONLY(pContext->SetDispSurface(_pRenderSurface));
