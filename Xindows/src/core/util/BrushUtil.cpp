...
//
//-----------------------------------------------------------------------------
HRESULT InitBrushCache(THREADSTATE* pts)
{
    Assert(pts);

    pts->pbcr = new BCR[CBCR_MAX];
    if(!pts->pbcr)
    {
...
//
//----------------------------------------------------------------------------
void DeinitBrushCache(THREADSTATE* pts)
{
    int     c;
    BCR*    pbcr;
...
//
//----------------------------------------------------------------------------
HBRUSH GetCachedBrush(COLORREF cr)
{
    THREADSTATE*    pts;
    int             c;
    BCR*            pbcr;
    ...
    if(!hbr)
    {
        Trace0("GCB: could not create brush, falling back to stock brush\n");

        // BUGBUG: Do we need to be careful sharing stock objects between threads?
        hbr = (HBRUSH)GetStockObject(BLACK_BRUSH); // 不必使用DeleteObject释放但是无碍
    }
...
void ReleaseCachedBrush(HBRUSH hbr)
{
    ...
    for(c=CBCR_MAX,pbcr=pts->pbcr; c>0; c--,pbcr++)
    {
        if(pbcr->hbr == hbr)
        {
            ...
            Trace3("GCB: released %x @ %d (%d)\n", pbcr->cr, pbcr-pts->pbcr, pbcr->refs);

            return;
        }
    }

    Trace0("RCB: Brush not found in cache, deleting anyway.\n");
